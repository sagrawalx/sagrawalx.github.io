...
\subsubsection*{Regular level set theorem}

First up, we have the following important theorem which goes by many names; we will call it the ``regular level set theorem,'' but some other names include the ``preimage theorem'' or the ``submersion theorem.'' We will later generalize this result. 
...
\begin{exercise} \label{torus-equation}
	...
	Thus, applying \cref{regular-level-set-theorem-local}, we can conclude that $T$ is a submanifold of $\R^3$. 
	\begin{hint}
