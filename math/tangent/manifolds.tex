...
\begin{definition}[Chart] \index{chart}
	...
	\begin{itemize}
		...
		\item If $p \in X$, we say that the chart $(U,x)$ \emph{contains} $p$ if $p \in U$. If $(U,x)$ contains $p$ and $x(p) = 0$, we say that $(U,x)$ is \emph{centered at} $p$. 
		\item For all $i = 1, \dotsc, n$, we define $x_i := \pi_i \circ x$. For any $p \in U$, the real numbers $x_1(p), \dotsc, x_n(p)$ are called the \emph{coordinates of $p$ with respect to the chart  $(U,x)$}.\index{coordinates}
	\end{itemize} 
\end{definition}
...
\begin{example} \label{polar-representation}
	...
	\[ U = \{(x,y) : x < 0 \text{ or } y \neq 0 \}. \]
	Let $\rho : U \to \R^2$ be the function which sends a point $p$ to its polar representation $\rho(p) = (r(p), \theta(p))$, where $r(p) = |p|$ and $\theta(p)$ is the angle in radians strictly between 0 and $2\pi$ formed between $p$ and the non-negative real axis. Then $(U, \rho)$ is a chart. 
	
	To see this, observe that $\rho$ is injective, we have
	\[ \rho(U) = \{ (r, \theta) : r > 0, \theta \in (0, 2\pi) \}, \]
	and the inverse function of $\rho$ is given by  $\rho^{-1}(r,\theta) = (r\cos \theta, r\sin\theta)$. Clearly $\rho^{-1}$ is continuous. In fact, it is even \'etale, since 
	\[ \det (\rho^{-1})'(r, \theta) = \det \begin{bmatrix} \cos \theta & -r\sin\theta \\ \sin\theta & r\cos \theta \end{bmatrix} = r^2 \neq 0 \]
	on $\rho(U)$, so the inverse function theorem \ref{inverse-function-theorem} tells us that $\rho^{-1}$ is open. This means that $\rho$ is continuous as well, proving that $(U, \rho)$ is a chart. 
	
	Notice that the chart $(U, \rho)$ contains the point $(1,1)$. We calculate the coordinates of $(1,1)$ with respect to this chart by evaluating $\rho(1,1)$. We find
	\[ \rho(1,1) = (\sqrt{1^2 + 1^2}, \arctan(1)) = (\sqrt{2}, \pi/4). \]
	In other words, $\rho$ outputs precisely the polar representation of $(1,1)$. 
\end{example}
...
\begin{example} \label{cube-not-compatible}
	Let $\id : \R \to \R$ denote the identity map and let $h : \R \to  \R$ denote the map $h(x) = x^3$. Both maps are homeomorphisms, so $(\R, \id)$ and $(\R, h)$ are charts. However, these two charts are not compatible. The transition function from $(\R, \id)$ to $(\R, h)$ is 
	\[ \begin{tikzcd} \R = \id(\R) \ar{r}{\id^{-1}} & \R \ar{r}{h} & h(\R) = \R, \end{tikzcd} \]
	which is just $h$, which is in fact smooth. However, in the other direction, the transition function is 
	\[ \begin{tikzcd} \R = h(\R) \ar{r}{h^{-1}} & \R \ar{r}{h} & \id(\R) = \R, \end{tikzcd} \]
	which is the function $h^{-1}(x) = \sqrt[3]{x}$. In other words, the transition function from $(\R, h)$ to $(\R, \id)$ is not differentiable. 
\end{example}

\begin{exercise}
	Determine whether or not each of the following charts $(U, h)$ is compatible with $(\R, \id)$.
	\begin{enumerate}[(a)]
		\item $U = \R$ and $h(x) = 2x$.
		\item $U = \R$ and $h(x) = -x$. 
		\item $U = \R$ and $h(x) = x|x|$. 
		\item $U = \R$ and $h(x) = -x^3$. 
		\item $U = (-\pi/2, \pi/2)$ and $h(x) = \tan(x)$. 
		\item $U = (0, \infty)$ and $h(x) = \ln(x)$. 
		\item $U = \R$ and $h(x) = x^5$. 
	\end{enumerate}
	Use these examples to formulate a conjecture for determining when a chart $(U, h)$ on $\R$ is compatible with $(\R, \id)$. Then prove your conjecture. 
\end{exercise}

\begin{example} \label{polar-compatible-with-identity}
	Let $\id : \R^2 \to \R^2$ denote the identity map and $\rho : U \to \R^2$ the polar representation map from \cref{polar-representation}. The two charts $(\R^2, \id)$ and $(U, \rho)$ are compatible. To see this, observe that the transition function from $(U, \rho)$ to $(\R^2, \id)$ is the function
	\[ \begin{tikzcd} \rho(U) \ar{r}{\rho^{-1}} & U \ar{r}{\id} & \id(U) = U, \end{tikzcd} \]
	which is just $\rho^{-1}$. We saw in \cref{polar-representation} that $\rho^{-1}$ is given explicitly by \[ \rho^{-1}(r, \theta) = (r\cos\theta, r\sin\theta), \]
	which is a smooth function. Going the other way, the transition function from $(\R^2, \id)$ to $(U, \rho)$ is precisely $\rho$. 
	\[ \begin{tikzcd} U = \id(U) \ar{r}{\id^{-1}} \ar{r} & U \ar{r}{\rho} & \rho(U) \end{tikzcd} \]
	As we saw in \cref{polar-representation}, the map $\rho^{-1}$ is \'etale, so its inverse function $\rho$ is smooth. Thus both transition functions are smooth, so the two charts are compatible. 
\end{example}

\begin{exercise}
	Show that two charts $(U,x)$ and $(U', x')$ are compatible if and only if the transition function from $(U, x)$ to $(U',x')$ is smooth and \'etale. 
\end{exercise}
...
\begin{remark} \label{compatibility-not-equivalence-relation}
	An important cautionary observation is that compatibility of charts is \emph{not} an equivalence relation on the set of all charts. It is certainly reflexive and symmetric, but it fails to be transitive. For example, let $h(x) = x^3$ and then consider the three charts $(\R, \id), (\R \setminus \{0\}, h)$, and $(\R, h)$. The first two are compatible with each other, and the second two are compatible with each other, but the first and third are \emph{not} compatible with each other. See \cref{cube-not-compatible}. 
\end{remark}
...
\begin{exercise} \label{dimension-well-defined}
	...
	\begin{hint}
		Pick a point $p \in U \cap V$, and observe that the two transition functions between $(U, x)$ and $(U', x')$ are inverses of each other. What can you say about the total derivatives of the transition functions at $x(p)$ and $x'(p)$?
	\end{hint}
\end{exercise}
...
\begin{definition}[Atlas] \index{atlas} \index{atlas!maximal atlas} \index{maximal atlas|see {atlas, maximal atlas}}
	An \emph{atlas} on $X$ is a collection $\mathscr{A}$ of compatible charts such that covers $X$. In other words, a collection $\mathscr{A}$ of charts is an atlas if every pair of charts in $\mathscr{A}$ is compatible, and \[ \bigcup_{(U, x) \in \mathscr{A}} U = X. \]
...
\begin{definition}[Dimension] \index{dimension!of a manifold}
	Suppose $X$ is a manifold and $p \in X$ is a point. By \cref{dimension-well-defined}, there is a unique integer $n$ such that $\dim(U,x) = n$ for every chart $(U, x) \in \mathscr{A}_X$ containing $p$. This integer is called the \emph{dimension of $X$ at $p$}, and is denoted $\dim_p(X)$. If there exists a single integer $n$ such that $\dim_p(X) = n$ for all $p \in X$, we say that $X$ is \emph{equidimensional} and that the \emph{dimension of $X$} is $n$. 
\end{definition}

\begin{proposition}
	If the set $\N$ of natural numbers is given the discrete topology, the function $X \to \N$ given by $p \mapsto \dim_p(X)$ is continuous. In particular, if $X$ is a nonempty connected manifold, it must be equidimensional. 
\end{proposition}

\begin{proof}
	To show that $p \mapsto \dim_p(X)$ is continuous, we want to show that, for any natural number $n$, the set 
	\[ X_n = \{ p \in X : \dim_p(X) = n \} \]
	is an open subset of $X$. Suppose $p \in  X_n$. Let $(U, x)$ be a chart containing $p$. Then $\dim(U,x) = n$, and we have $\dim_q(X) = n$ for all $q \in U$. In other words, $U \subseteq X_n$. Thus every point in $X_n$ is contained inside an open neighborhood that is entirely contained in $X_n$, so $X_n$ is open. 
	
	Now suppose $X$ is connected and $p \in X$. Let $n = \dim_p(X)$, so that $p \in X_n$. Then 
	\[ X \setminus X_n = \bigcup_{m \neq n} X_m \]
	is open, so $X_n$ is a nonempty subset of $X$ that is both open and closed. Since $X$ is connected, we conclude that $X = X_n$. In other words, $X$ is equidimensional of dimension $n$. 
\end{proof}
...
\begin{example} \label{natural-euclidean-atlas}
	The single chart $(\R^n, \id)$, where $\id : \R^n \to \R^n$ is the identity map, defines an atlas on $\R^n$. We can extend this to a maximal atlas, called the \emph{euclidean atlas} on $\R^n$. Unless explicitly specified otherwise, we always regard $\R^n$ as a manifold by equipping it with the euclidean atlas. By \cref{polar-compatible-with-identity}, the polar representation chart $(U, \rho)$ is an element of the euclidean atlas. 
\end{example}

\begin{exercise}
	For any $r > 0$, let $h_r : \R \to \R$ be the function
	\[ h_r(t) = \begin{cases} t & \text{if } t \leq 0 \\ rt & \text{if } t > 0. \end{cases} \]
	Note that $h_r$ is continuous and strictly increasing (hence also injective and open, by \cref{strictly-monotone-iff-injective,strictly-monotone-implies-open}), so $(\R, h_r)$ is a chart. Note also that $h_1 = \id$. 
	\begin{enumerate}[(a)]
		\item Show that $h_s \circ h_r = h_{sr}$ and that $h_r^{-1} = h_{1/r}$. 
		\item Show that $(\R, h_r)$ and $(\R, h_s)$ are compatible if and only if $r = s$.
		\item Let $\mathscr{A}_r$ be the maximal atlas obtained by extending $(\R, h_r)$. What can you say about $\mathscr{A}_r$ and $\mathscr{A}_s$ if $r \neq s$? Are any of these the euclidean atlas? What can you conclude about the number of maximal atlases on $\R$?
	\end{enumerate}  
\end{exercise}

\begin{exercise}
	Let $h : \R \to \R$ be the function $h(t) = t^3$. Note that $h$ is continuous and strictly increasing, so $(\R, h)$ is a chart (by \cref{strictly-monotone-iff-injective,strictly-monotone-implies-open}). Show that $(\R, f)$ is \emph{not} a member of the euclidean atlas on $\R$. 
\end{exercise}
...
\begin{exercise}[The circle $S^1$] \index{s1@$S^1$|see {circle}} \index{circle} \label{circle-projection-atlas}
	...
	\begin{enumerate}[(a)]
		...
		\item Let $U = S^1 \setminus \{(0,1)\}$. For $p \in S^1$, let $x(p)$ denote the $x$-coordinate of the point where the line $y = -1$ intersects the line passing through $(0,1)$ and $p$. This is sometimes called ``projection from the north pole.'' See \cref{circle-projection}. If $p = (a, b)$, find a formula for $x(p)$ in terms of $a$ and $b$. 
		...
		\begin{figure}
			\begin{center}
				...
				\end{tikzpicture}
			\end{center}
			\caption{Given a point $p \in U = S^1 \setminus \{(0,1)\}$, we draw a line connecting $p$ to $(0,1)$, indicated in red. The point where that line intersects the line $y = -1$ (in black) is sometimes called the ``projection'' of $p$ from $(0,1)$ onto $\R \times \{-1\}$. The $x$-coordinate of this point is $x(p)$.}  \label{circle-projection}
		\end{figure}
		...
		\item Can you think of any other charts in the maximal atlas $\mathscr{A}_{S^1}$, besides the three discussed above? In other words, can you think of any other charts that are compatible with both $(U,x)$ and $(U',x')$?
		
		\begin{hint}
			There are many, many possibilities here. You could look at functions $(a,b) \mapsto a$ and/or $(a,b) \mapsto b$ on other halves of the circle, besides the right half discussed in (f) above. More interestingly, you could also think about charts defined somehow using sines and cosines. 
		\end{hint}
	\end{enumerate}
\end{exercise}
...
\begin{example}[Matrices]
	...
	If we endow $\mathscr{M}_{n \times m}$ with the canonical topology,\index{canonical topology} then $x$ is automatically a homeomorphism by \cref{linear-isomorphism-homeomorphism}.\index{homeomorphism} In other words, the single chart $(\mathscr{M}_{n \times m}, x)$ is an atlas on $\mathscr{M}_{n \times m}$. Extending it to a maximal atlas using \cref{existence-maximal-atlas}, we see that we can regard $\mathscr{M}_{n,m}$ as a manifold. It is $mn$-dimensional.  
\end{example}
...
Let $B$ denote the box $[0,1] \times (0,1)$ inside $\R^2$, and let $\sim$ be the equivalence relation on $B$ generated by declaring $(0,b) \sim (1,1-b)$ for all $b \in (0,1)$. 
See \cref{mobius-box}. The equivalence class of a point $(a,b) \in B$ is denoted by $[a,b]$. Observe that every equivalence class contains either 1 or 2 elements. The \emph{M\"obius strip}\index{mobius strip@M\"obius strip} $M$ is the set of equivalence classes in $B$. In other words, we take the box $[0,1] \times (0,1)$ and ``glue'' its left and right edges, with a twist. If you've ever made a M\"obius strip out of paper, you can probably see that we've formalized exactly that same process. See \cref{mobius-box}, again.
...
\begin{solution}{\cref{first-chart-mobius}}
	Let $\pi : B \to M$ be the map $\pi(a,b) = [a,b]$. To show that $U = \{[a,b] \in M : a \neq 0,1\}$ is open, we use the definition of the quotient topology \cref{quotient-space}, which tells us that $U$ is an open subset of $M$ precisely when $\pi^{-1}(U)$ is an open subset of $B$. But \[ \pi^{-1}(U) = B^\circ = (0,1) \times (0,1) \] is an open subset of $B$, so $U$ is open in $M$. 
	
	The facts that $x : U \to \R^2$ is well-defined and injective follow immediately from the observation that every equivalence class in $U$ has exactly one element. To see that $x$ is continuous, suppose $W$ is an open subset of $\R^2$. We want to show that $x^{-1}(W)$ is open in $U$, ie, that $\pi^{-1}(x^{-1}(W))$ is open in $B$. But $\pi^{-1}(x^{-1}(W)) = B^\circ \cap W$ is an intersection of two open subsets, so it is also open. 
	
	To see that $x$ is open, suppose $W$ is an open subset of $U$. We want to show that $x(W)$ is open in $\R^2$. But notice that $x(W) = \pi^{-1}(W)$, and we know that $\pi^{-1}(W)$ is open in $B$ by the definition of open subsets in the quotient topology. Moreover, $x(W) \subseteq B^\circ$, so $x(W)$ is an open subset of $B^\circ$. Since $B^\circ$ is open in $\R^2$, we conclude that $x(W)$ is open in $\R^2$. 
\end{solution}
...
\begin{solution}{\cref{second-chart-mobius}}
	Let $\pi : B \to M$ be the map $\pi(a,b) = [a,b]$ again. Observe that $\pi^{-1}(V) = \{ (a,b) \in B : a \neq 1/2 \}$  is an open subset of $B$ (even though it is not an open subset of $\R^2$), so by definition of the quotient topology, $V$ is an open subset of $M$. 
	
	Let us check that $y$ is well-defined. 
	The verification that $y$ is continuous, injective, and open will be left to the reader. Most equivalence classes have just one element, and $y$ is clearly well-defined on those equivalence classes. Equivalence classes with two points are of the form $[0,b] = [1,1-b]$ for some $b \in (0,1)$. Observe that 
	\[ y([1,1-b]) = (0,1-(1-b)) = (0,b) = y([0,b]), \]
	which shows that $y$ is well-defined on the equivalence classes with two elements as well. 
\end{solution}
...
\begin{solution}{\cref{mobius-charts-compatible}}
	Observe that the transition function from $(U,x)$ to $(V,y)$ is the composite 
	\[ \begin{tikzcd} x(U \cap V) \ar{r}{x^{-1}} & U \cap V \ar{r}{y} & y(U \cap V) \end{tikzcd} \]
	given by 
	\[ (y \circ x^{-1})(a,b) = y([a,b]) = \begin{cases} (a,b) & \text{if } a < 1/2 \\ (a-1, 1-b) & \text{if } a > 1/2 \end{cases} \]
	which is clearly smooth, and 
	\[ (y \circ x^{-1})'(a,b) = \begin{cases} \begin{bmatrix} 1 & 0 \\ 0 & 1 \end{bmatrix} & \text{if } a < 1/2 \\ \begin{bmatrix} 1 & 0 \\ 0 & -1 \end{bmatrix} & \text{if } a > 1/2 \end{cases} \]
	which shows that $y \circ x^{-1}$ is \'etale. Thus the two charts are compatible. 
\end{solution}
...
\begin{exercise}[Cylinder] \label{cylinder}
	Let $S$ denote the infinite vertical strip $[0,1] \times \R$ inside $\R^2$, and let $\sim$ be the equivalence relation generated by $(0,b) \sim (1,b)$ for all $b \in \R$. Let $C$ be the corresponding quotient space. Describe an atlas on $C$. 
\end{exercise}
...
\begin{exercise}
	Let $C$ be the cylinder from \cref{cylinder} and let $L$ be the line \[ L = \{ [1/2, b] \in C : b \in \R \}. \] Show that $L$ is a submanifold of $C$. 
\end{exercise}
...
\subsubsection*{Submersion theorem}

First up, we have the following important theorem which goes by many names, including the ``regular level set theorem,'' the ``preimage theorem'' and the ``submersion theorem.'' We will later generalize this result. 

\begin{theorem}[Submersion theorem] \label{submersion-theorem-local}
	Suppose $U$ is an open subset of $\R^m$ and $f : U \to \R^n$ is smooth and submersive. Then $S = f^{-1}(q)$ is a submanifold of $U$ of dimension $m-n$ for any $q \in \R^n$. 
\end{theorem}

\begin{proof}
	By replacing $f$ with the function $x \mapsto f(x)-q$, we can assume without loss of generality that $q = 0$. Fix $p \in S$. We want to find a chart adapted to $S$ containing $p$. We know that $f$ is submersive at $p$, which means that the $n \times m$ matrix $f'(p)$ has a non-vanishing $n \times n$ minor, ie, there exist $n$ distinct integers $i_1, \dotsc, i_n$ between $1$ and $m$ such that 
...
\begin{exercise}
	...
	\begin{enumerate}[(a)]
		\item Use the submersion theorem \ref{submersion-theorem-local} to show that $S^1$ is a submanifold of $\R^{2}$. 
...
\begin{exercise} \label{torus-equation}
	Let $T$ be the torus inside $\R^3$ centered around the $z$-axis where the distance from the origin to the center of the tube is $R$, the radius of the tube is $r$, and $0 < r < R$. Find a smooth function $f : U \to \R$ where $U$ is an open subset of $\R^3$, 0 is a regular value of $f$, and \[ T = \{ (x,y,z) \in U : f(x,y,z) = 0 \}. \]
	Thus, applying the submersion theorem \ref{submersion-theorem-local}, we can conclude that $T$ is a submanifold of $\R^3$. 
	\begin{hint}
...
\subsubsection*{Graphs of smooth functions}
...
\begin{proof}
	We derive this from the submersion theorem \ref{submersion-theorem-local}. Consider the function $g : U \times \R^n \to \R^n$ given by $g(x, y) = f(x) - y$. Then the graph of $f$ is equal to $g^{-1}(0)$, so the result will follow from the submersion theorem provided we can show that $g$ is submersive. To see this, we calculate $dg$. Note that if $x = (x_1, \dotsc, x_m) \in U$ and $y = (y_1, \dotsc, y_n) \in \R^n$, then
...
\begin{caution}
	Sometimes, a subspace that is a manifold can fail to be a submanifold. For example, you might notice that we showed in \cref{graphs-continuous} that the graph of a \emph{continuous} function between two euclidean spaces is a subspace of euclidean space and is also a manifold; and then we showed in \cref{graphs-smooth} that the graph of a \emph{smooth} function is a submanifold of euclidean space. And indeed, the graphs of continuous-but-not-smooth functions provide examples of subspaces that are manifolds but not submanifolds. 
\end{caution}
...
\begin{exercise}
	Suppose $U$ is an open subset of $\R^m$ and $f : U \to \R^n$ is a smooth function. We have discussed two atlases on the graph $\Gamma$ of $f$. One was discussed in \cref{graphs-continuous}, and the other arises by combining \cref{graphs-smooth,submanifolds-are-manifolds}. How do these atlases compare?
\end{exercise}
