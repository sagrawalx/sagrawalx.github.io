...
\begin{definition}[Chart] \index{chart}
	A \emph{chart} on $X$ is a pair $(U, x)$ consisting of an open subset $U \subseteq X$ and a function $x : U \to \R^n$ which is a homeomorphism onto an open subset of $\R^n$ for some $n$.\index{homeomorphism!homeomorphism onto its image} We call the functions $x_i := \pi_i \circ x : U \to \R$ \emph{coordinate functions} on $U$ with respect to the chart $(U, x)$, and for any $p \in U$, the list of numbers $x_1(p), \dotsc, x_n(p)$ are the \emph{coordinates} of $p$ with respect to the chart $(U, x)$.\index{coordinates} 
\end{definition}
...
\begin{definition} \label{transition-map} \index{transition function}
	Suppose $(U, x)$ and $(V, y)$ are charts on $X$. The \emph{transition function} from $(U, x)$ to $(V, y)$ is the function $y \circ x^{-1}$. 
	\[ \begin{tikzcd}
	x(U \cap V) \ar{r}{x^{-1}} & U \cap V \ar{r}{y} & y(U \cap V)
	\end{tikzcd} \]
\end{definition}

\begin{definition}[Compatibility of charts] \index{compatibile}
...
\begin{exercise} \label{compatibility-not-equivalence-relation}
	Show that the relation ``is compatible with'' is \emph{not} an equivalence relation on the set of all charts on $X$. 
	\begin{hint}
...
\begin{exercise} \label{dimension-well-defined}
	Suppose $(U, x)$ and $(V, y)$ are compatible charts, where $x : U \to \R^m$ and $y : V \to \R^n$, and that $U \cap V$ is nonempty. Show that $m = n$. 
	\begin{hint}
		Pick a point $p \in U \cap V$, and consider the derivatives of $x \circ y^{-1}$ at $y(p)$ and of $y \circ x^{-1}$ at $x(p)$. 
	\end{hint}
\end{exercise}
...
\begin{exercise}[Maximal atlases] \label{existence-maximal-atlas}
...
\begin{definition}[Dimension] \index{dimension!of a manifold}
	Suppose $X$ is a manifold and $p \in X$ is a point. By \cref{dimension-well-defined}, there is a unique integer $n$ such that, for every chart $(U, x) \in \mathscr{A}_X$ with $p \in U$, the codomain of $x$ is $\R^n$. This integer is called the \emph{dimension of $X$ at $p$}, and is denoted $\dim_p(X)$. We say that $X$ is \emph{$n$-dimensional} if $\dim_p(X) = n$ for all $p \in X$.
\end{definition}
...
\begin{exercise}[The circle $S^1$] \index{s1@$S^1$|see {circle}} \index{circle} \label{circle-projection-atlas}
	...
	\begin{enumerate}[(a)]
		\item There are two ways you might think to regard $S^1$ as a topological space. First, you might restrict the euclidean metric on $\R^2$ to $S^1$, so that $S^1$ becomes a metric space, and then you can forget this restricted metric and just remember the resulting topology as in \cref{metric-to-topological}. Alternatively, you can first forget the metric on $\R^2$ and regard it as a topological space as in \cref{metric-to-topological}, and then regard $S^1$ as a subspace of $\R^2$ (cf. \cref{subspace-topology}).\index{subspace} Show that you end up with the same topology on $S^1$ in both cases. 
		...
		\item Let $U' = S^1 \setminus \{(0,-1)\}$. For $p \in V$, let $x'(p)$ denote the $x$-coordinate of the point where the line $y = 1$ intersects the line passing through $(0,-1)$ and $p$. Show that $x' : U' \to \R$ is also a homeomorphism. 
		
		\item Show that $(U, x)$ and $(U', x')$ are compatible charts.
	\end{enumerate}
	Thus $\mathscr{A} = \{(U, x), (U', x')\}$ is an atlas on $S^1$. Extending $\mathscr{A}$ to a maximal atlas, we have defined a manifold structure on $S^1$. It is evidently a 1-dimensional manifold. 
\end{exercise}
...
\begin{example}[Matrices]
	...
	If we endow $\mathscr{M}_{n \times m}$ with the canonical topology,\index{canonical topology} then $x$ is automatically a homeomorphism.\index{homeomorphism} In other words, the single chart $(\mathscr{M}_{n \times m}, x)$ is an atlas on $\mathscr{M}_{n \times m}$. Extending it to a maximal atlas using \cref{existence-maximal-atlas}, we see that we can regard $\mathscr{M}_{n,m}$ as a manifold. It is $mn$-dimensional. 
\end{example}
...
The \emph{M\"obius strip}\index{mobius strip@M\"obius strip} is the set of equivalence classes $M := B/\sim$. In other words, we take the box $[0,1] \times (0,1)$ and ``glue'' its left and right edges, with a twist. If you've ever made a M\"obius strip out of paper, you should be able to see that we've formalized exactly that same process. We regard $M$ as a topological space using the quotient topology, as in \cref{quotient-space}. 
...
\begin{exercise}
	Show that $y : V \to \R^2$ is well-defined and that it is a homeomorphism onto its image.\index{homeomorphism!homeomorphism onto its image} 
\end{exercise}
...
We will now define what it means for a function between two manifolds to be smooth. Recall that in \cref{multi} we saw that the case when the codomain is $\R$ was the most important. The same will be true here. 
...
\begin{exercise}
	Suppose $U \subseteq \R^m$ is an open subset and $f : U \to \R$ is a function. Show that $f$ is smooth in the sense of \cref{smooth-multivar} if and only if it is smooth in the sense of \cref{smooth-real-valued}. 
	\begin{hint}
		You might find \cref{smooth-real-val-local} useful. 
	\end{hint}
\end{exercise}
...
\begin{exercise}
	Show that $\O(X)$ is a vector space. 
\end{exercise}

\begin{exercise}
	If $f, g \in \O(X)$, show that $fg \in \O(X)$ also. 
\end{exercise}

\begin{proposition}
	Suppose $K$ is a compact subset of $X$ and $U$ is an open neighborhood of $K$. Then there exists a smooth function $f : X \to [0,1]$ such that $f(x) = 1$ for all $x \in K$ and $f(x) = 0$ for all $x \notin U$. 
\end{proposition}
