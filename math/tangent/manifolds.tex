...
\begin{exercise}[The circle $S^1$] \label{circle-projection-atlas}
	...
	\begin{enumerate}[(a)]
		\item There are two ways you might think to regard $S^1$ as a topological space. First, you might restrict the euclidean metric on $\R^2$ to $S^1$, so that $S^1$ becomes a metric space, and then you can forget this restricted metric and just remember the resulting topology as in \cref{metric-to-topological}. Alternatively, you can first forget the metric on $\R^2$ and regard it as a topological space as in \cref{metric-to-topological}, and then regard $S^1$ as a subspace of $\R^2$ (cf. \cref{subspace-topology}). Show that you end up with the same topology on $S^1$ in both cases. 
		...
		\item Show that $x : U \to \R$ is a homeomorphism. Thus $(U, x)$ is a chart on $S^1$. 
		\begin{hint} 
...
\begin{example}[Matrices]
	There is an isomorphism of vector spaces $x : \mathscr{M}_{n \times m} \to \R^{mn}$ where, if $A \in \mathscr{M}_{n \times m}$ has entries $a_{i,j}$, then
	\[ x(A) = \begin{bmatrix} a_{1,1} \\ a_{1,2} \\ \vdots \\ a_{1,m} \\ a_{2,1} \\ \vdots \\ a_{n,m} \end{bmatrix}. \]
	We regard $\mathscr{M}_{n \times m}$ as a topological space using the matrix norms defined in \cref{matrix-norm}, then $x$ is a homeomorphism by definition. In other words, the single chart $(\mathscr{M}_{n \times m}, x)$ is an atlas on $\mathscr{M}_{n \times m}$. Extending it to a maximal atlas using \cref{existence-maximal-atlas}, we see that we can regard $\mathscr{M}_{n,m}$ as a manifold. It is $mn$-dimensional. 
\end{example}
...
\begin{definition}[Smooth real-valued function] \label{smooth-real-valued}
