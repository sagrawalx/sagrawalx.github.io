...
\begin{exercise} \label{dimension-well-defined}
	Suppose $(U, x)$ and $(U', x')$ are compatible charts, where $x : U \to \R^m$ and $x' : U' \to \R^n$, and that $U \cap U'$ is nonempty. Show that $m = n$. 
	\begin{hint}
...
\begin{exercise} \label{smooth-stable-manifold}
	\begin{enumerate}[(a)]
...
We now prove that there is a preponderance of bump functions on manifolds. 

\begin{proposition}
	Suppose $K$ is a compact subset of $X$ and $U$ is an open neighborhood of $K$. Then there exists a smooth function $f : X \to \R$ such that $f(x) \in [0,1]$ for all $x \in \R$, $f(x) = 1$ for all $x \in K$, and $f(x) = 0$ for all $x \notin U$. 
\end{proposition}

\begin{proof}
	For each $p \in K$, let $(V, x)$ be a chart containing $p$. By shrinking $V$, we can assume that $\bar{V} \subseteq U$. 
\end{proof}
