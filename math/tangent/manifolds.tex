...
We'll justify this terminology in a moment by showing that submanifolds are actually manifolds in their own right (cf. \cref{submanifolds-are-manifolds}). But first, let's discuss some examples. The intuition is roughly that, if a subset $S$ of $X$ is ``smooth'' and doesn't have any ``pointy'' parts, it should be a submanifold. 
...
\begin{exercise} \label{circle-submanifold}
	...
	\begin{hint}
		Let $U = \{(x,y) \in S^1 : x > 0 \}$ and let $\phi(x,y) = (x^2+y^2-1, y)$. Show that $(U, \phi)$ is a chart that is adapted to $S^1$. But this one chart does not cover the circle, so then find more charts like this one that cover the circle. 
	\end{hint}
\end{exercise}
