...
We now define submanifolds. Roughly, a submanifold is a subset for which there exist charts along such that the subset looks like the coordinate hyperplanes inside that chart. Before making the formal definition, recall that, if $(U, \phi)$ is a chart, then $\phi_i = \pi_i \circ \phi$ denotes the corresponding $i$th coordinate function. 
...
\begin{definition} \label{submanifold}
	...
	\[ S \cap U = \{ x \in U :  \phi_1(x) = \phi_2(x) = \dotsb = \phi_k(x) = 0 \} \]
	for some non-negative integer $k \leq \dim(U,x)$. Such a chart $(U,\phi)$ is said to be \emph{adapted to $S$}, or is a \emph{slice chart for $S$}. The integer $k$ is called the \emph{codimension of $S$ in $(U,\phi)$}. 
\end{definition}

% TODO: Picture

We'll justify this terminology in a moment by showing that submanifolds are actually manifolds in their own right (cf. \cref{submanifolds-are-manifolds}). But first, let's discuss some examples. The intuition is roughly that, if a subset $S$ of $X$ is ``smooth'' (ie, doesn't have any ``pointy'' parts), it should be a submanifold. 

\begin{example}
	Consider the set $S = \{ (0,y) \in \R^2 : -1 < x < 1 \}$ inside $\R^2$. In other words, $S$ is the open interval $(-1,1)$ along the $y$-axis inside $\R^2$. Then $S$ is a submanifold. Indeed, let $U$ be the open box $(-1,1) \times (-1,1)$ inside $\R^2$, and $\phi : U \to \R^2$ the inclusion map. Then $(U, \phi)$ is a chart on $\R^2$, $\phi_1(x,y) = x$, and 
	\[ \{ (x,y) \in U : \phi_1(x, y) = 0 \} = \{ (x,y) \in U : x = 0 \} = S = S \cap U \]
	so $(U,\phi)$ is adapted to $S$ and $S$ has codimension 1 in $(U,\phi)$. 
\end{example}

\begin{example}
	Consider the parabola \[ S = \{ (x, y) \in \R^2 : y = x^2, x \in \R \}. \] This doesn't ahve any ``pointy'' parts, so it should be a submanifold. Let's see how. It is sufficient to produce a chart $(\R^2, \phi)$ with the property that 
	\[ S = \{ (x,y) \in \R^2 : \phi_1(x,y) = 0 \}. \]
	This means we should take $\phi_1(x,y) = x^2 - y$. Then $\phi_2$ should be a smooth function $\phi_2 : \R^2 \to \R$ such that $\phi = (\phi_1, \phi_2)$ is injective and \'etale. In other words, we want the Jacobian matrix 
	\[ \phi'(x,y) = \begin{bmatrix} 2x & -1 \\ \dfrac{\partial \phi_2}{\partial x} & \dfrac{\partial \phi_2}{\partial y} \end{bmatrix} \]
	to always be invertible. The easiest way to guarantee this is to take $\phi_2(x,y) = x$, which makes $\det \phi'(x,y) = 1$. Moreover, the function $\phi(x,y) = (x^2 - y, x)$ is injective, since if $\phi(x_1, y_1) = \phi(x_2,y_2)$, then looking at the second coordinates shows that $x_1 = x_2$, and then looking at the first coordinates shows that $y_1 = y_2$. In other words, $(\R^2, \phi)$ is a chart in the euclidean atlas which covers $S$ and is adapted to $S$, so $S$ is a submanifold of $\R^2$. 
\end{example}
...
\begin{exercise}
	...
	\begin{enumerate}[(a)] 
		...
		\item The graph of any smooth function $f : \R \to \R$.  
	\end{enumerate}
\end{exercise}
...
\begin{exercise} \label{circle-submanifold}
	...
	\begin{hint}
		Let $U = \{(x,y) \in S^1 : x > 0 \}$ and let $\phi(x,y) = (x^2+y^2-1, y)$. See \cref{circle-submanifold-picture}. Show that $(U, \phi)$ is a chart that is adapted to $S^1$. But this one chart does not cover the circle, so then find more charts like this one that cover the circle. 
	\end{hint}
	\begin{figure}
		\begin{center}
			\begin{tikzpicture}
			...
			\addplot[white] expression[name path=c,domain=0:2,samples=2]{1.5};
			\addplot[white] expression[name path=d,domain=0:2,samples=2]{-1.5};
			\addplot[fill=gray, fill opacity=0.1]
			fill between[of=c and d,soft clip={domain=0:2}];
			
			\addplot[red,thick] expression[domain=0:1,samples=200]{sqrt(1-x^2)};
			\addplot[red,thick] expression[domain=0:1,samples=200]{-sqrt(1-x^2)};
			
			\addplot +[mark=none,thick,black,dashed,->] coordinates {(0, -1.5) (0, 1.5)};
			
			\node[color=white,draw=red,circle,fill,inner sep=2pt] at (axis cs:0,1) {}; 
			\node[color=white,draw=red,circle,fill,inner sep=2pt] at (axis cs:0,-1) {}; 
			...
			\addplot[black,thick,dashed,->] expression[name path=a,domain=-1:1.15,samples=200]{sqrt(x+1)};
			
			\addplot[black,thick,dashed,-] expression[name path=b,domain=-1:1.15,samples=200]{-sqrt(x+1)};
			
			\addplot[fill=gray, fill opacity=0.1]
			fill between[of=a and b,soft clip={domain=-1:1.15}];
			
			
			\addplot[white] expression[name path=c,domain=1.15:2,samples=2]{1.5};
			\addplot[white] expression[name path=d,domain=1.15:2,samples=2]{-1.5};
			\addplot[fill=gray, fill opacity=0.1]
			fill between[of=c and d,soft clip={domain=1.15:2}];
			
			\addplot +[mark=none,red,thick,solid,-] coordinates {(0, -1) (0, 1)};
			...
			\end{tikzpicture}
		\end{center}
		\caption{If $U = \{(x,y) : x > 0\}$ is the right half of the plane and $\phi : U \to \R^2$ is given by $\phi(x,y) = (x^2 + y^2 - 1, x)$, then $(U, \phi)$ is a chart on $\R^2$ that transforms the picture on the left into the picture on the right. The vertical axis on the left turns into the parabola on the right. The red semicircle on the left turns into the red line segment along the vertical axis on right. The horizontal axis on the left stays remains the horizontal axis on the right.}  \label{circle-submanifold-picture}
	\end{figure}
\end{exercise}
...
\begin{proposition} \label{submanifolds-are-manifolds}
	Let $S$ be a submanifold of a manifold $X$. Suppose $(U, \phi) \in \mathscr{A}_X$ is adapted to $S$ and $S$ has codimension $k$ in $(U, \phi)$. Let $\phi|_S : S \cap U \to \R^{n-k}$ be the function defined by 
	\[ \phi|_S(p) = (\phi_{k+1}(p), \dotsc, \phi_n(p)). \]
	Regarding $S$ as a topological space using the subspace topology, the pair $(S \cap U, \phi|_S)$ is a chart on $S$, and the set of all charts of this form is an atlas on $S$. Thus $S$ becomes a manifold if we equip it with the corresponding maximal atlas $\mathscr{A}_S$. Finally, if $(U, \phi) \in \mathscr{A}_X$ is a chart adapted to $S$ such that $S$ has codimension $k$ in $S$, then
	\[ \dim_x(S) = \dim_x(X) - k \]
	for all $x \in S \cap U$. 
\end{proposition}

\begin{proof}
	Observe that, by definition, $\phi|_S$ is the composite
	\[ \begin{tikzcd}
	S \cap U \ar[hookrightarrow]{r} & U \ar{r}{\phi} & \R^n \ar{r}{\pi} & \R^{n-k}
	\end{tikzcd} \]
	...
	Let us show that $\phi|_S$ is open. By definition of the subspace topology \ref{subspace-topology}, every open subset of $S \cap U$ is of the form $S \cap V$ where $V$ is an open subset of $U$. Suppose $a \in  S \cap V$. We want to show that $\phi|_S(a)$ is an interior point of $\phi|_S(S \cap V)$. Since $V$ is open in $U$ and $\phi : U \to \R^n$ is an open map, we know that $\phi(a)$ is an interior point of $\phi(V)$, ie, there exists $\epsilon > 0$ such that every point of $\R^n$ within $\epsilon$ of $\phi(a)$ is inside $\phi(V)$. We claim that the $\epsilon$ ball around $\phi|_S(a)$ is contained in $\phi|_S(S \cap V)$. Suppose we have $y = (y_{k+1}, \dotsc, y_n) \in \R^{n-k}$ such that $|y - \phi|_S(a)| < \epsilon$. We will show that $y \in \phi|_S(S \cap V)$. 
	
	Notice that, since $(U,\phi)$ is adapted to $S$, the vector $\phi(a) \in \R^n$ is the same as $\phi|_S(a) \in \R^{n-k}$ except that it is prepended with $k$ zeroes. Let $\iota : \R^{n-k} \to \R^n$ be the map that preprends vectors in $\R^{n-k}$ with a string of $k$ zeroes. In other words, 
	\[ \iota(y_{k+1}, \dotsc, y_n) = (\overbrace{0, \dotsc, 0}^{k \text{ times}}, y_{k+1}, \dotsc, y_n). \]
	Thus $\phi(a) = \iota(\phi|_S(a))$. Now by the definition of the either the euclidean or the max norm, we see that $|\iota(y) - \phi(a)| = |y - \phi|_S(a)|$, which means that $|\iota(y) - \phi(a)| < \epsilon$. This means that $\iota(y) \in \phi(V)$ due to our choice of $\epsilon$. Since $\phi$ is injective, there exists a unique $b \in V$ such that $\phi(b) = \iota(y)$. This means that the first $k$ coordinates of $b$ are zero, so $b \in S \cap U$ since $(U, \phi)$ is adapted to $S$. Moreover
	\[ \phi|_S(b) = \pi(\phi(b)) = \pi(\iota(y)) = y \]
	which shows that $y \in \phi|_S(S \cap V)$. 
	
	Next up, suppose $(U, \phi)$ and $(U', \phi')$ are both charts in $\mathscr{A}_X$ that are adapted to $S$. We want to show that $(S \cap U, \phi|_S)$ and $(S \cap U', \phi'|_S)$ are compatible. If $S \cap U \cap U'$ is empty, there is nothing to do, so we assume that $S \cap U \cap U'$ is nonempty. Let $k$ and $k'$ be the codimensions of $S$ in the two charts $(U, \phi)$ and $(U', \phi')$, respectively. Once we prove compatibility, it follows from \cref{dimension-well-defined} that we must have $k = k'$. 
	...
	\[ \mathscr{A} = \{ (S \cap U, \phi|_S) : (U, \phi) \in \mathscr{A}_X \text{ is adapted to } S \} \]
...
\begin{exercise}[``Open submanifolds are submanifolds''] \label{open-submanifolds-are-submanifolds}
	...
	\end{enumerate}
	\begin{hint}
		Show that a chart $(V, \phi) \in \mathscr{A}_X$ is adapted to $U$ if and only if $V \subseteq U$, and that $U$ has codimension 0 in any such chart. 
	\end{hint}
\end{exercise}
...
\begin{proof}
	By replacing $f$ with the function $x \mapsto f(x)-b$, we can assume without loss of generality that $b = 0$. Fix $a \in S$. We want to find a chart adapted to $S$ containing $p$. We know that $f$ is submersive at $a$, which means that the $n \times m$ matrix $f'(a)$ has a non-vanishing $n \times n$ minor, ie, there exist $n$ distinct integers $i_1, \dotsc, i_n$ between $1$ and $m$ such that 
	\begin{equation} \label{derivative-non-vanishing-minor} 
	\det \begin{bmatrix} \partial_{i_1} f_1(a) & \dotsb & \partial_{i_n} f_1(a) \\ \vdots & \ddots & \vdots \\ \partial_{i_1} f_n(a) & \dotsb & \partial_{i_n} f_n(a) \end{bmatrix} \neq 0. 
	\end{equation}
	By permuting the coordinates, we can assume without loss of generality that $i_1 = 1, i_2 = 2, \dotsc, i_n = n$ (cf. \cref{permutation-submanifold}). Consider the function $\phi : U \to \R^m$ such that, if $x = (x_1, \dotsc, x_m) \in U$, then
	\[ \phi(x) = (f_1(x), \dotsc, f_n(x), x_{n+1}, \dotsc, x_m). \]
	Then
	\[ \phi'(x) = 
	\begin{bmatrix} 
	...
	0 & \dotsb & 0 & 0 & \dotsb & 1
	\end{bmatrix} \]
	In other words, the $(m-n) \times m$ submatrix on the bottom left of $\phi'(x)$ is all zeroes, and the $(m-n) \times (m-n)$ submatrix on the bottom right is the identity matrix. This means that $\det y'(x)$ is equal, up to sign, to the determinant of the $n \times n$ in the top left, which we know from \cref{derivative-non-vanishing-minor} is nonzero when $x = a$. Thus $\phi$ is \'etale at $a$. 
	
	Since $f$ is smooth, so is $\phi$, so there exists an open neighborhood $V$ of $a$ such that $\phi|_V$ is \'etale. By the inverse function theorem \ref{inverse-function-theorem}, we can replace $V$ with a smaller open neighborhood of $a$ in order to further assume that $\phi|_V$ is injective. Then \cref{euclidean-chart-characterization} tells us that $(V, \phi|_V)$ is a chart in the euclidean atlas which contains $a$. Moreover, 
	\[ S \cap V = \{ x \in V : f(x) = 0 \} = \{ x \in V : \phi_1(x) = \dotsb = \phi_n(x) = 0 \} \]
	because $\phi_j = f_j$ for all $j = 1, \dotsc, n$. This shows that $(V,\phi|_V)$ is adapted to $S$, and that $S$ has codimension $n$ at $a$ in $\R^m$. 
\end{proof}
