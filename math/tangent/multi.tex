...
Then
\[ \epsilon(h,k) = |(2+h)^2 + (1+k)^2 - 5 - 4h - 2k| = h^2 + k^2. \]
Notice that $\epsilon$ gets small rapidly as $(h,k) \to 0$. More precisely, the claim is that
\[ \lim_{(h,k) \to 0} \frac{\epsilon(h,k)}{|(h,k)|} = 0, \]
ie, that $\epsilon(h,k) = o(|(h,k)|)$ as $(h,k) \to 0$. 

In fact, for the claim, it doesn't matter whether $|(h,k)|$ denotes the euclidean norm $|(h,k)|_2 = \sqrt{h^2 + k^2}$ or the max norm $|(h,k)|_\mx = \max\{|h|, |k|\}$. If it's the euclidean norm, we have
\[ \lim_{(h,k) \to 0} \frac{\epsilon(h,k)}{|(h,k)|_2} = \lim_{(h,k) \to 0} = \frac{h^2 + k^2}{\sqrt{h^2 + k^2}} = \lim_{(h,k) \to 0} \sqrt{h^2 + k^2} = 0. \]
If instead it's the max norm, observe that
\[ \frac{\epsilon(h,k)}{|(h,k)|_\mx} = \frac{h^2 + k^2}{|(h,k)|_\mx} \leq \frac{2(|(h,k)|_\mx)^2}{|(h,k)|_\mx} = 2|(h,k)|_\mx \]
so taking the limit as $(h,k) \to 0$ and applying the squeeze theorem yields the same result. 

Since $\epsilon$ gets small rapidly, we can say that the function $(h,k) \mapsto f(a) + df_a(h,k)$ is a good approximation of the function $(h,k) \mapsto f(a+(h,k))$ for small vectors $(h,k)$. Said differently, letting $x = a + (h,k)$, the function $x \mapsto f(a) + df_a(x-a)$ is a good approximation of $f$ near $a$. 
...
We'll be using a lot of linear algebra in this chapter. The notation $|-|$ will denote a norm on $\R^n$. For the most part, it won't matter whether it's the euclidean norm $|h|_2 = \sqrt{h_1^2 + \dotsb + h_2^2}$ or the max norm $|h|_\mx = \max\{|h_1|, \dotsc, |h_n|\}$. You can choose whichever one you like better. If it makes a difference which norm on $\R^n$ we have in mind, we'll explicitly write either $|-|_2$ or $|-|_\mx$. 

We'll also need some facts about the operator norm; I encourage you to at least skim through \cref{operator-norm-basic} before proceeding. 
...
\begin{exercise} \label{derivative-of-multiplication}
	Let $\mu : \R^2 \to \R$ be the multiplication map $\mu(x, y) = xy$. Prove that
	\[ d\mu_{(a,b)}(h, k) = bh + ak \]
...
\begin{proof}
	We'll prove this for the situation when $|-|$ denotes the max norm. For the proof when $|-|$ is the euclidean norm, see \cite[theorem 9.19]{rudin}. 
	
	Let us suppose first that $n = 1$. If $a = b$, there is nothing to do, so we can assume that $a \neq b$. Let \[ h = \frac{b-a}{|b-a|_\mx}, \]
	...
	\[ |(f \circ \xi)'(t)| = |df_{a + th}(h)| \leq \|df_{a + th}\| |h|_\mx \leq M|h|_\mx = M, \]
	...
	\[ |f(b) - f(a)| \leq M||b-a|_\mx - 0| = M|b-a|_\mx.  \]
	This proves the statement for $n = 1$, and using this we can now prove the statement for general $n$ (still assuming that $|-|$ is the max norm $|-|_\mx$). Note that $df_{j,a} = \pi_j \circ df_a$ and $\|\pi_j\| = 1$ so \[ \|df_{j,a}\| \leq \|\pi_j\| \|df_a\| \leq M \]
	...
	\[ |f(b) - f(a)| \leq M|b-a|. \]
	Recall that, so far, $|-|$ denoted the max norm. 
...
which is evidently contradiction. In other words, we have proved that $a$ is an interior point of the domain of $\sigma$. 
...
The second argument is a little less quick, but in the end it's very similar to the first argument; the main reason for including it here is that it's somewhat similar in spirit to the general proof. The idea is to set $b := f(a_1) = f(a_2)$ and consider the function \[ \sigma(x) = (f(x)-b)^2. \]
Since $\sigma$ is a continuous function on the compact interval $[a_1, a_2]$, the extreme value theorem guarantees that it attains its maximum at some point $\xi \in [a_1, a_2]$. Observe that $\sigma(a_1) = \sigma(a_2) = 0$ and that $\sigma \geq 0$. If the maximum occurred at one of the endpoints, then $\sigma = 0$ constantly, so $f$ is constantly equal to $b$, which means the derivative vanishes on $(a_1, a_2)$ by \cref{constant-iff-derivative-zero}, contradicting our assumption that $f$ is \'etale. Thus $\xi$ is an interior point of the interval $(a_1, a_2)$. Then the interior extremum theorem \ref{interior-extremum} plus the chain rule tells us that
\[ 0 = \sigma'(\xi) = 2f'(\xi),  \]
which again contradicts our assumption that $f$ is \'etale. 
