...
% TODO: Remarks about why its important
...
\begin{example}
	...
	\[ \begin{aligned} x + y^2 &= a \\ y - xy^2 + 3x^2 &= b \end{aligned} \]
	When $a = b = 0$, this system has a solution given by $x = y = 0$. What if $a$ and $b$ are not both zero? Does this system have any solutions? Are the solutions unique at all? None of this is very obvious on the face of it. 
	
	Let $f : \R^2 \to \R^2$ be the function given by $f(x,y) = (x+y^2, y - xy^2 + 3x^2)$. Then 
	\[ \det f'(x,y) = \det \begin{bmatrix} 1 & 2y \\ 6x -y^2 & 1 - 2xy \end{bmatrix} = (1 - 2xy) - 2y(6x - y^2) = 1 - 14xy + 2y^3.  \]
...
\begin{exercise}
	Let $U = \{(x,y) : x > 0\}$ and let $f : U \to \R^2$ be the function \[ f(x,y) = (xe^{-y}, xe^y). \]
	\begin{enumerate}[(a)]
...
The proof of the multinomial theorem is omitted because we won't actually have much use for the theorem. If you're interested, try it yourself; if you get stuck, you can find a proof on \href{https://en.wikipedia.org/wiki/Multinomial_theorem}{Wikipedia}. 
...
Here is some practice using the statement of Taylor's theorem. 

\begin{exercise}
	Let $f : \R^2 \to \R$ be a $C^2$ function. Suppose $a \in \R^2$ is a critical point of $f$ and $\partial^{(1,1)}f(a) = 0$. Use the degree 2 Taylor polynomial of $f$ at $a$ to formulate a rule for determining when $a$ is a local minimum, local maximum, or neither, and prove your rule. 
\end{exercise}
