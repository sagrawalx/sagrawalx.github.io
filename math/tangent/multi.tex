...
See \cref{multi-introductory-example-graph-tangent-lines}.
...
These two tangent lines uniquely determine the entire tangent plane $T$ (ie, $T$ is the plane containing these two lines). One way of describing $T$ that immediately falls out of the parametrizations we've found of the tangent lines is the plane parametrized by
\[ \begin{bmatrix} h \\ k \end{bmatrix} \mapsto \begin{bmatrix} 2 \\ 1 \\ 5 \end{bmatrix} + \begin{bmatrix} h \\ k \\ 4h + 2k \end{bmatrix}. \]
The important part of this expression is the bottom entry on the far right, the $4h + 2k$. The function $\R^2 \to \R$ given by $(h, k)  \mapsto 4h + 2k$ is what we will call the \emph{total derivative} of $f$ and denote by $df_a$. 

Notice that $df_a$ is a linear map $\R^2 \to \R$. Moreover, its graph is a plane passing through the origin that is parallel to $T$. In other words, if we take the graph of $df_a$ and translate it over to the point $(2,1,5)$, we obtain exactly the tangent plane $T$. Speaking more loosely, the derivative $df_a : \R^2 \to \R$ records all of the ``slopey information'' about the tangent plane $T$. 
...
Then
\[ \epsilon(h,k) = |(2+h)^2 + (1+k)^2 - 5 - 4h - 2k| = |h^2 + k^2| = |(h,k)|^2. \]
Notice that $\epsilon$ gets small rapidly as $(h,k) \to 0$. More precisely, notice that
\[ \lim_{(h,k) \to 0} \frac{\epsilon(h,k)}{|(h,k)|} = \lim_{(h,k) \to 0} |(h,k)| = 0, \]
so $\epsilon(h,k) = o(|(h,k)|)$ as $(h,k) \to 0$. Since $\epsilon$ gets small rapidly, we can say that the function $(h,k) \mapsto f(a) + df_a(h,k)$ is a good approximation of the function $(h,k) \mapsto f(a+(h,k))$ for small vectors $(h,k)$. Said differently, letting $x = a + (h,k)$, the function $x \mapsto f(a) + df_a(x-a)$ is a good approximation of $f$ near $a$. 

\subsection*{Overview}

In what follows, we will turn the above example on its head and \emph{define} the derivative using approximations. In other words, the derivative $df_a$ will be defined to be a linear function which yields a good approximation of $f$ near a point $a$, and then we will prove in \cref{jacobian-matrix} that partial derivatives (ie, derivatives along various ``slices'') can be used to compute the standard matrix representation of the derivative. 

This might seem a bit ``backwards'' given our analysis of the example above, but there is a good reason for doing this; it turns out that there are some bizarre functions where partial derivatives make sense, but tangent planes do not (cf. \cref{partials-dont-imply-differentiable,directionals-dont-imply-differentiable}).

As you may be able to tell, we'll be using linear algebra heavily in this chapter. We'll also need some facts about the operator norm; I encourage you to at least skim through \cref{operator-norm-basic} before proceeding. 
...
\begin{definition}[Derivative at a point] \index{derivative!derivative at a point} \index{total derivative|see {derivative}} \index{differential|see {derivative}}
	Suppose $f : S \to \R^n$ is differentiable at an interior point $a \in S$. Then the \emph{total derivative of $f$ at $a$}, denoted $df_a$, is the unique linear map $\R^m \to \R^n$ satisfying
	\[ |f(a+v)-f(a)-df_a(h)| = o(|h|) \text{ as } h \to 0.  \]
	Sometimes $df_a$ is also called the \emph{differential} of $f$ at $a$. 
\end{definition}

We've now defined the derivative, but it's fairly difficult to use this definition in practice. There are only a few examples that can be computed directly from this definition. Here are some.
...
Of course, the converse to \cref{differentiable-implies-continuous} is false. We've seen single variable examples; here is a multivariable example. 
...
\begin{exercise}[Quotient rule] \label{quotient-rule} \index{quotient rule}
	...
	\begin{hint}
		Combine \cref{reciprocal-rule,product-rule}. 
	\end{hint}
\end{exercise}
...
\begin{exercise} \label{all-directionals-vanish-but-not-extremum}
	...
	\begin{enumerate}[(a)]
		...
		\item Show that $f$ has an absolute minimum when restricted to an arbitrary line through the origin. Conclude that $\partial_h f(a) = 0$ for all $h \in \R^2$. 
	\end{enumerate}
\end{exercise}

Now for the main result of this section: partial derivatives help us compute derivatives. Note that we will generalize this result slightly in \cref{jacobian-matrix} below. 
...
Now for the important caveat: unfortunately, the converse to \cref{differentiable-implies-directionals} is \emph{not} true: the existence of all partial derivatives is not sufficient to guarantee differentiability. In fact, even the existence of all \emph{directional} derivatives is not sufficient to guarantee differentiability. 
...
\begin{exercise} \label{partials-dont-imply-differentiable}
	...
	Show that the partial derivatives of $f$ with respect to $x$ and $y$ both exist at the origin, but that $f$ is not even continuous at the origin (so it cannot be differentiable, by \cref{differentiable-implies-continuous}). 
	\begin{hint} Approach the origin along the line $y = rx$ for some nonzero $r$. See \cref{partials-dont-imply-differentiable-graph}. \end{hint}
	\begin{figure}[ht]
		\begin{center}
			\begin{tikzpicture}
			\begin{axis}[
...
\begin{exercise} \label{directionals-dont-imply-differentiable}
	...
	\end{figure} 
	Show that $f$ is continuous and that $\partial_h f(0)$ exists for all nonzero $h \in \R^2$, but that $f$ is not differentiable at the origin. 
	\begin{hint}
		To show that $f$ is not differentiable at the origin, suppose it were differentiable, and then find a vector $h \in \R^2$ such that 
		\[ [df_0]h \neq \partial_h f(0), \]
		which contradicts \cref{differentiable-implies-directionals}. 
	\end{hint}
\end{exercise}

Thankfully, this phenomenon is fairly pathological; \cref{continuous-partials} will provide a converse to \cref{differentiable-implies-directionals} under fairly mild additional hypotheses. 
...
\begin{definition}[Rank of a differentiable map] \label{immersive-submersive-critical} \index{rank!of a differentiable function}
	...
	\begin{itemize}
		...
		\item If $b \in \R^n$ is a point such that every $a \in f^{-1}(b)$ is a regular point, then $b$ is called a \emph{regular value} of $f$. 
	\end{itemize}
\end{definition}
...
\begin{exercise}[Parametrization of a cusp] \label{cusp}
	...
	\end{figure}
	Calculate $f'(a)$ for all $a \in \R$. Show that 0 is a critical point of $f$, and that $f$ is immersive away from 0.
\end{exercise}
...
\begin{exercise}[Parametrization of a node] \label{node}
	...
	\end{figure}
	Calculate $f'(a)$ for all $a \in \R$, and show that $f$ is immersive everywhere.
\end{exercise}
...
Throughout this section, $U$ denotes an open subset of $\R^m$. In this section, we'll discuss functions that are differentiable at every point in $U$. 

\begin{definition} \index{differentiable} \index{derivative}
	A function $f : U \to \R^n$ is \emph{differentiable} if it is differentiable at every $a \in U$. The \emph{total derivative} of $f$, denoted $df$, is the function $U \to \mathscr{L}(\R^m, \R^n)$ given by $a \mapsto df_a$. Sometimes the function $df$ is also called the \emph{differential} of $f$. 
\end{definition}
...
Here is a multivariable generalization of \cref{constant-iff-derivative-zero-single}. In fact, we will use the single variable version to prove this multivariable version. 
...
\begin{proof}
	Taking $w = 0$ in \cref{derivative-of-line} shows that, if $f$ is constant, then $df = 0$. For the converse, we can assume without loss of generality that $n = 1$ (cf. \cref{constant-iff-derivative-zero-details}). We want to show that, for any $a,  b \in B$, we have $f(a) = f(b)$. Consider the function $\gamma : [0,1] \to B$ given by
	\[ \gamma(t) = tb + (1-t)a.  \]
...
In fact, it's not necessary that the domain be an open ball in order for the conclusion of \cref{constant-iff-derivative-zero} to be true. Before getting to the general statement, here is a useful intuition-building exercise. 

\begin{exercise}
	Let $U$ denote $\R^2$ minus the non-negative $x$-axis. In other words,
	\[ U = \R^2 \setminus \{ (x, 0) \in \R^2 : x \geq 0 \}. \]
	Suppose $f : U \to \R$ is a differentiable function such that $df = 0$. Show that $f$ is constant. 
	\begin{hint}
		In the proof of \cref{constant-iff-derivative-zero}, we used the fact that any pair of points in an open ball could be joined by a straight line path that stayed entirely within the open ball. It's not true that any pair of points in $U$ can be joined by a straight line path that stays entirely within $U$. But any pair of points in $U$ \emph{can} be joined by \emph{two} straight line paths...
	\end{hint}
\end{exercise}
...
\begin{proposition} \label{bounded-derivative-multi}
	Suppose $B$ is an open ball and $f : B \to \R^n$ is a differentiable function. Suppose further that there exists a constant $M$ such that $\|df_x\| \leq M$ for all $x \in B$. Then
	\[ |f(b) - f(a)| \leq \sqrt{n} M |b-a| \]
	for all $a, b \in B$. 
\end{proposition}

\begin{proof}
	Let us suppose first that $n = 1$. If $a = b$, there is nothing to do, so we can assume that $a \neq b$. Let \[ h = \frac{b-a}{|b-a|}, \]
	...
	This proves the statement for $n = 1$, and using this we can now prove the statement for general $n$. Note that $df_{j,a} = \pi_j \circ df_a$ and $\|\pi_j\| = 1$ so \[ \|df_{j,a}\| \leq \|\pi_j\| \|df_a\| \leq M \]
	by \cref{operator-norm-submultiplicative}. Applying \cref{bounded-derivative-multi} to the component functions $f_j$, so 
	\[ |f_j(b) - f_j(a)| \leq M|b-a| \]
	for all $a, b \in B$ and all $j = 1, \dotsc, n$, which means that
	\[ |f(b) - f(a)|_\mx \leq M|b-a|, \]
	where $|-|_\mx$ denotes the max norm (cf.  \cref{euclidean}). But then
	\[ |f(b)-f(a)| \leq \sqrt{n} |f(b)-f(a)|_\mx \leq \sqrt{n}M |b-a|, \]
	where we have used \cref{max-l2} for the first step.
\end{proof}

\begin{exercise}
	\begin{enumerate}[(a)]
		\item The statement of the \cref{bounded-derivative-multi} requires that the domain of $f$ is an open ball. Where in the proof is this used?
		\item Prove that $\|\pi_j\| = 1$. 
...
\begin{comment}
	...
	So we can assume that $n = 1$. Set $h = b-a = (h_1, \dotsc, h_m)$, and then define the following list of vectors. 
	\[ \begin{aligned} 
	a_0 &= a \\
	...
	We're now going to restrict $f$ to the zigzag path from $a$ to $b$ made by the points $a_0, \dotsc, a_m$, looking at each segment in turn. The function
...
\begin{definition}
	A function $f : U \to \R^n$ is \emph{\'etale} if it is differentiable and \'etale at every $a \in U$. 
\end{definition}
...
\begin{exercise} \label{locally-injective-not-injective}
	...
	\begin{enumerate}[(a)]
		...
		\item Show that $f$ is \emph{not} injective. In fact, for any point $(a,b) \in \R^2$, show that there exist infinitely many other points in the domain with the same image as $(a,b)$.
...
\begin{definition}
	...
	\begin{itemize}
		...
		\item We say that $f$ is \emph{locally surjective at $a$} if $f(a)$ is an interior point of $f(V)$ for every neighborhood $V$ of $a$. 
	\end{itemize}
\end{definition}
...
Suppose $y \in B(0, r/3)$ and consider the function $\sigma : \overline{B}(0,r) \to \R$ given by $\sigma(x) = |f(x)-y|$. Since $\overline{B}(0,r)$ is compact, this function must attain its minimum at a point $x \in \overline{B}(0,r)$. We will show that $f(x) = y$. 

In fact, we must have $x \in B(0,r)$. Suppose for a contradiction that $|x| = r$. Then by \cref{inverse-function-theorem-eqn1} we would have
\[ r/2 \geq |f(x)-x| \geq ||x| - |f(x)|| = r - |f(x)| > 2r/3, \]
which is evidently contradiction. In other words, $x$ is an interior point of the domain of $\sigma$. 

Now we prove that we must have $f(x) = y$. Suppose for a contradiction that $f(x) \neq y$. Then the euclidean norm function $|-| : \R^n \to \R$ is differentiable at $f(x) - y$, which means that $\sigma$ is differentiable at $x$ by the chain rule \cref{chain-rule}, and
\[ d\sigma_x = d(|-|)_{f(x)-y} \circ df_x. \]
Since the interior point $x$ is the absolute minimum of $\sigma$, the interior extremum theorem \ref{interior-extremum-directional} and \cref{jacobian-matrix} tell us that $d\sigma_x = 0$, which means that $df_x(\R^n) \subseteq \ker(d(|-|)_{f(x)-y})$. This is a contradiction: since $df_x$ is invertible, we have $\dim df_x(\R^n) = n$, whereas $\dim(\ker(d(|-|)_{f(x)-y})) = n-1$ by the rank-nullity theorem. 

We have now proved \cref{inverse-function-theorem-locally-surjective}. Notice that this implies that $f$ is locally surjective at 0. Indeed, any open neighborhood $V$ of 0 contains an open ball of the form $B(0,r)$ for $0 < r < 1$, and then \[ f(V) \supseteq f(B(0,r)) \supseteq B(0,r/3), \] so $f(0) = 0$ is an interior point of $f(V)$. 
...
Suppose we have $x_1 < x_2$ such that $f(x_1) = f(x_2)$. We'll prove that this leads to a contradiction using two different but very similar arguments. The first argument is very quick. The mean value theorem guarantees that there exists $\xi$ between $x_1$ and $x_2$ such that $f'(\xi) = 0$, which contradicts our assumption that $f$ is \'etale. 

The second argument is a little less quick, but in the end it's very similar to the first argument; the main reason for including it here is that it's somewhat similar in spirit to the general proof. The idea is to set $y := f(x_1) = f(x_2)$ and consider the function $\sigma(x) = |f(x) - y|$. Since $\sigma$ is a continuous function on the compact interval $[x_1, x_2]$, the extreme value theorem guarantees that it attains its maximum at some point $\xi \in [x_1, x_2]$. If $\sigma(\xi) = 0$, then $\sigma$ is identically zero, which means that $f$ is constantly equal to $y$ on $[x_1, x_2]$, and then \cref{constant-iff-derivative-zero} tells us that we've contradicted our assumption that $f$ is \'etale. Thus $\sigma(\xi) > 0$. But then $\sigma$ is differentiable at $\xi$, so the interior extremum theorem \ref{interior-extremum} guarantees that $\sigma'(\xi) = 0$. The chain rule implies that $f'(\xi) = 0$, again contradicting the assumption that $f$ is \'etale. 
...
Let $V = B(0,\delta)$. The claim is that $f|_V$ is injective. 

Let $g(x) = x - f(x)$, and observe that $\partial_i g_j(x) = \delta_{i,j} - \partial_i f_j(x)$, so we have 
\[ \|g'(x)\|_\mx = \max_{i,j} |\partial_i g_j(x)| \leq \frac{1}{2n} \]
for all $x \in V$. Using \cref{operator-and-matrix-norms}, we see that $\|dg_x\| \leq 1/2$ for all $x \in V$, so \cref{bounded-derivative-multi} tells us that 
\[ |g(x_1) - g(x_2)| \leq \frac{1}{2} |x_1 - x_2| \]
for all $x_1, x_2 \in V$. But
\[ |g(x_1) - g(x_2)| = |(x_1 - f(x_1)) - (x_2 - f(x_2))| \geq |x_1 - x_2| - |f(x_1) - f(x_2)| \]
by the reverse triangle inequality. Thus we have the following inequalities. 
...
{\color{blue} Incomplete. I'll write this up eventually. Meanwhile, if you're really curious, look at \href{https://terrytao.wordpress.com/2011/09/12/the-inverse-function-theorem-for-everywhere-differentiable-maps/#more-5298}{Terry Tao's blog post}.}
...
The following theorem is the promised sort-of-converse to \cref{differentiable-implies-directionals}. Recall from \cref{partials-dont-imply-differentiable} above that $\partial_i f$ existing for all $i$ is \emph{not} sufficient to guarantee differentiability. But, insisting that $\partial_i f$ exist \emph{and be continuous} turns out to be enough to guarantee differentiability of $f$. The proof is harder than you might expect. 
...
\begin{exercise} \label{no-continously-differentiable-bijection}
	...
	\begin{hint}
		Suppose for a contradiction that $f : \R^2 \to \R$ is a continuously differentiable bijection.  Let $U$ be an open subset of $\R^2$ such that $\partial_1 f(x) \neq 0$ for all $x \in U$, and then consider the function $g : U \to \R^2$ given by $g(x_1, x_2) = (f(x_1, x_2), x_2)$. 
	\end{hint}
\end{exercise}
...
For this section, we will need to make substantial use of the continuity properties of the operator norm.\index{operator norm} This might make it a bit challenging for those who have not used the operator norm before, but the results contained in this section are also not terribly important. The only reason this section is included is because \cref{continuously-differentiable} is aesthetically a rather satisfying statement which can sometimes streamline proofs. 
...
\begin{proof}
	...
	\[ \partial_{j,i} f(a) = \lim_{h \to 0} \sigma(t,t). \]
	...
	Applying the mean value theorem \ref{mean-value-theorem}\index{mean value theorem} to $\alpha$ on the closed interval between 0 and $h$, there exists $\xi_i(t)$, strictly between 0 and $t$, such that
	...
	so, applying the mean value theorem \ref{mean-value-theorem}\index{mean value theorem} to $\beta$ on the closed interval between 0 and $h$, we find that there exists $\xi_j(t)$, strictly between 0 and $t$, such that
	\[ \sigma(t,t) = \frac{\beta(t) - \beta(0)}{t} = \beta'(\xi_j(t)) = \partial_{j,i} f(a + \xi_i(t) e_i + \xi_j(t) e_j). \]
	Now notice that, since both $\xi_i(t)$ and $\xi_j(t)$ are between 0 and $h$, we have $\xi_i(t), \xi_j(t) \to 0$ as $h \to 0$. Since $\partial_{j,i} f$ is continuous at $a$, we have
...
The main difficulty in the multivariable Taylor's theorem is notation; we've already done most of the hard work of the proof with the single variable versions of \cref{taylor-single,taylor-single-remainder-lagrange}. Let us first discuss some standard notational conventions that make the statement of Taylor's theorem more readable. 
...
We now establish some rules of doing algebra with multi-indices. 
...
\begin{exercise}[Equality of mixed partials, multi-index version] \label{mixed-partials-multi-index}
	...
	\begin{hint}
		First consider the case when $\alpha = 1_i$ for some $i = 1, \dotsc, m$. 
	\end{hint}
\end{exercise}

\begin{theorem}[Multinomial theorem] \label{multinomial}
	For any $x = (x_1, \dotsc, x_m) \in \R^m$, we have 
	\[ (x_1 + \dotsb + x_m)^\ell = \sum_{|\alpha| = \ell} \binom{\ell}{\alpha} x^\alpha. \]
\end{theorem} 

The proof of the multinomial theorem is omitted because we won't actually have much use for it. If you're interested, try it yourself; if you get stuck, you can find a proof on \href{https://en.wikipedia.org/wiki/Multinomial_theorem}{Wikipedia}. 

There is a convenient combinatorial interpretation of $\binom{\ell}{\alpha}$ when $\ell = |\alpha|$ which generalizes the interpretation of binomial coefficients. For this reason, the $\binom{\ell}{\alpha}$ are often called ``multinomial coefficients.''\index{multinomial}

\begin{proposition} \label{multinomial-combinatorial}
...
\begin{theorem}[Taylor] \label{taylor}
	Suppose $B$ is an open ball, $f : B \to \R$ is $C^k$ for some non-negative integer $k$, and $a \in U$. Then there exists a unique polynomial $p_k$ of degree at most $k$ in $m$ variables, called the \emph{degree $k$ Taylor polynomial of $f$ at $a$}, such that $|f(a+h) - p_k(h)| = o(|h|^k)$ as $h \to 0$. Moreover, we have
	\begin{equation} \label{taylor-polynomial} p_k(h) = \sum_{|\alpha| \leq k} \frac{\partial^\alpha f(a)}{\alpha!} h^\alpha. \end{equation}
	Finally, if $f$ is $C^{k+1}$, then for any $h$ such that $a + h \in B$, there exists $\xi$ on the line segment between $a$ and $a+h$ such that 
	\[ f(a+h) - p_k(h) = \sum_{|\alpha| = k+1} \frac{\partial^\alpha f(\xi)}{\alpha!} h^{k+1}. \]
\end{theorem}

While the notation is compact and aesthetically pleasing, there's a lot of information packed into very few symbols. I exhort you to work through the following example before proceeding. It may start feeling tedious at some point, but power through it. 

\begin{exercise}
	Let $f : \R^2 \to \R$ be the function $f(x,y) = \sin(xy)$. Calculate the degree 3 Taylor polynomial of $f$ at the origin. 
\end{exercise}
...
\begin{proof}[Proof of Taylor's theorem \ref{taylor}]
	...
	\[ |h^\alpha| = |h_1^{\alpha_1} \dotsb h_m^{\alpha_m}| \leq |h|_\mx^k \leq |h|^k, \]
	where we have used \cref{max-l2} for the final step. 
...
Recall that a ``bump function''\index{bump function} is a smooth function with compact support. We can use single variable bump functions to construct multivariable bump functions. 

\begin{exercise} \label{multivariable-bump-square-support}
	Let $\psi : \R \to \R$ denote the single variable bump function from \cref{canonical-bump}. Then define $\Psi : \R^m \to \R$ by
	\[ \Psi(x_1, \dotsc, x_m) = \psi(x_1) \psi(x_2) \dotsb \psi(x_m). \]
	See \cref{multivariable-bump-square-support-graph}. Show that $\Psi$ is a bump function with support $[-1,1]^m$. 
	\begin{figure}[ht]
		\begin{center}
			\begin{tikzpicture}
			\begin{axis}[
...
\begin{exercise} \label{multivariable-bump-round-support}
	Let $\psi : \R \to \R$ denote the single variable bump function from \cref{canonical-bump}. Then define $\Psi : \R^m \to \R$ by
	\[ \Psi(x_1, \dotsc, x_m) = \psi(x_1^2 + \dotsb + x_m^2). \]
	Show that $\Psi$ is a bump function with support $\overline{B}(0,1)$.  
\end{exercise}
...
\begin{definition}[Paths]
	...
	\begin{itemize}
		\item We say that $\gamma$ is \emph{smooth} if it is the restriction to $[0,1]$ of a smooth function $(-\epsilon, 1+\epsilon) \to \R^m$ for some $\epsilon > 0$. 
		\item We say that $\gamma$ is \emph{inside} or \emph{contained in} an open set $U$ if $\gamma(t) \in U$ for all $t \in [0,1]$.
	\end{itemize}
\end{definition}

For example, if $a, b \in \R^m$, we have seen the straight line path $\gamma(t) = (1-t)a + tb$ which starts at $a$ and ends at $b$ several times (eg, the proof of \cref{constant-iff-derivative-zero}). It is smooth. 
