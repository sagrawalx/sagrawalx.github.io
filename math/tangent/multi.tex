...
In this chapter, we will study derivatives of functions $S \to \R^n$ where $S$ is a subset of $\R^m$ and $m$ and $n$ are arbitrary positive integers. As it turns out, the most important case is when $n = 1$. In other words, the ``multi'' in the name of this chapter (as opposed to the ``single'' in the name of the previous chapter) is really referring to the fact that we might have multiple \emph{inputs} (ie, $m > 1$), rather than to the fact that we might have multiple outputs (ie, $n > 1$). Moreover,  the single variable case is actually quite important for the multivariable case; we'll often use results from \cref{single} to prove their multivariable counterparts. 
...
\begin{definition}[Directional derivatives] \index{directional derivative} \index{partial derivative}
	...
	If $h = e_i$ is the $i$th standard basis vector (cf. \cref{euclidean}), then this is called the \emph{$i$th partial derivative of $f$ at $a$} and denoted $\partial_i f(a)$ (instead of $\partial_{e_i} f(a)$). If we're using $x$ to denote the input to $f$ and $x_i$ to denote the $i$th component of the input, $\partial_i f(a)$ is sometimes also denoted by one of the following. 
...
\begin{proposition} \label{differentiable-implies-directionals}
	...
	Thus, the standard matrix representation $[df_a]$ of the linear map $df_a : \R^m \to \R$ is a $1 \times m$ matrix whose $i$th entry is $\partial_i f(a)$. 
...
\begin{exercise} \label{partials-dont-imply-differentiable}
	...
	\begin{hint} Approach the origin along the line $x = y$. See \cref{partials-dont-imply-differentiable-graph}. \end{hint}
	\begin{figure}[ht]
		\begin{center}
			\begin{tikzpicture}
			\begin{axis}[
...
Using \cref{jacobian-matrix}, many of the rules for differentiation we've seen acquire can be rewritten in forms that you might be more familiar with from single variable calculus. For example, since composition of linear maps corresponds to multiplication of the corresponding matrix representations, the chain rule \ref{chain-rule} states that 
...
\begin{exercise}[Parametrization of a cusp] \label{cusp}
	...
	\end{figure}
	Calculate $f'(a)$ for all $a \in \R$. Then show that 0 is a critical point of $f$, and that $f$ is immersive away from 0.
\end{exercise}
...
for all $|h| < \delta$. Replacing $f$ with the function $x \mapsto f(x/\delta)$, we can assume without loss of generality that $\delta = 1$ (cf. \cref{wlog-inverse-function-theorem}). 

We now claim that for all $0 < r < 1$, we have
\begin{equation} \label{inverse-function-theorem-locally-surjective} B(0,r/3) \subseteq f(B(0,r)). \end{equation}
Suppose $y \in B(0, r/3)$ and consider the function $\sigma : \overline{B}(0,r) \to \R$ given by $\sigma(x) = |f(x)-y|$. Since $\overline{B}(0,r)$ is compact, this function must attain its minimum at a point $x \in \overline{B}(0,r)$. 
...
There are many proofs of the inverse function theorem, but none of them are particularly easy. The proof we give here is the one from \cite[chapter 2]{spivak_calculus}. 
...
\begin{proof}[Proof of the inverse function theorem \ref{inverse-function-theorem}]
	...
	whereas
	\[ g(a) = |y - f(a)|^2 < d^2/4, \]
	so $g(a) < g(x)$, which contradicts the fact that $x$ is the minimum of $g$. Thus $x \in \bar{B} \setminus \partial B = B$. By the interior extremum theorem \ref{interior-extremum-directional}\index{extremum!interior extremum theorem}, we know that
...
The main difficulty in the multivariable Taylor's theorem is notation. Let us first discuss some notational conventions to make the statement of Taylor's theorem more readable. 
...
\begin{theorem}[Taylor] \label{taylor}
	Suppose $f : U \to \R$ is $C^k$ and $a \in U$. Then there exists a unique polynomial $p_k$ of degree $k$ in $m$ variables, called the \emph{degree $k$ Taylor polynomial of $f$ at $a$}, such that $|f(a+h) - p_k(h)| = o(|h|^k)$ as $h \to 0$. Moreover, we have
	\begin{equation} \label{taylor-polynomial} p_k(h) = \sum_{|\alpha| \leq k} \frac{\partial^\alpha f(a)}{\alpha!} h^\alpha. \end{equation}
\end{theorem}

\begin{proof}
	Replace $U$ with an open ball around $a$, and fix $h$ such that $a + h \in U$. Consider the function $\gamma(t) = a + th$, which is clearly $C^k$. Then $g = f \circ \gamma$ is a single variable $C^k$ function, so 
	\[ g(1) = \sum_{i = 0}^k \frac{g^{(i)}(0)}{i!} + r(1) \]
\end{proof}
