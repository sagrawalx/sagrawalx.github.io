...
\begin{exercise}[Multinomial theorem] \label{multinomial}
	...
	\begin{hint}
		You might use the combinatorial interpretation of  \cref{multinomial-combinatorial}. 
	\end{hint}
\end{exercise}
...
\begin{example}
	...
	\[ \begin{aligned} \partial^{(2,0)}f(1,0) &= \left.\frac{\partial^2 f}{\partial x^2}(1,0) = \frac{2y^2 - 2x^2}{x^2 + y^2}\right|_{(x,y) = (1,0)} = -2 \\
	\partial^{(1,1)}f(1,0) &= \frac{\partial^2 f}{\partial x^2}(1,0) = \left.\frac{-4xy}{x^2 + y^2}\right|_{(x,y) = (1,0)} = 0 \\
	\partial^{(0,2)}f(1,0) &= \frac{\partial^2 f}{\partial x^2}(1,0) = \left.\frac{2x^2 - 2y^2}{x^2 + y^2}\right|_{(x,y) = (1,0)} = 2 \end{aligned} \]
	This lets us assemble the degree 2 Taylor polynomial. 
	...
	Taylor's theorem \ref{taylor} states that \[ f(1+h,k) \approx 2h - h^2 + k^2 \] for small $(h,k)$. More precisely, the assertion is that the difference between these two functions is $o(|(h,k)|^2)$ as $(h,k) \to 0$. 
\end{example}

I exhort you to work through the following example yourself before proceeding. It may start feeling tedious at some point, but power through it. 
