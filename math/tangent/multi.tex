...
In fact, for the claim, it doesn't matter whether $|(h,k)|$ denotes the euclidean norm $|(h,k)|_2 = \sqrt{h^2 + k^2}$ or the max norm $|(h,k)|_\infty = \max\{|h|, |k|\}$ (cf. \cref{euclidean}). If it's the euclidean norm, we have
...
\[ \frac{\epsilon(h,k)}{|(h,k)|_\infty} = \frac{h^2 + k^2}{|(h,k)|_\infty} \leq \frac{2(|(h,k)|_\infty)^2}{|(h,k)|_\infty} = 2|(h,k)|_\infty \]
...
\begin{exercise} \label{components-are-small-implies-small}
	...
	\begin{hint}
		If you're using the euclidean norm, you may find it useful to do \cref{max-euclidean} first. 
	\end{hint}
\end{exercise}
...
First off, here are some definitions that are rather useful. 

\begin{definition} \label{straight-line-path}
	If $a, b \in \R^n$, the \emph{straight line path} from $a$ to $b$ is the function
	\[ \gamma(t) = (1-t)a + tb. \]
	Observe that $\gamma(0) = a$ and $\gamma(1) = b$. 
\end{definition}

\begin{exercise} \label{straight-line-path-differentiable}
	Show that the straight line path from $a$ to $b$ is differentiable. 
\end{exercise}

\begin{definition} \label{convex}
	A subset $S$ of $\R^n$ is \emph{convex} if the straight line path between any two points in $S$ is contained in $S$. In other words, $S$ is convex if, whenever $a, b \in S$ and $\gamma$ is the straight line path from $a$ to $b$, then $\gamma(t) \in S$ for all $t \in [0,1]$. 
\end{definition}

Roughly speaking, the role of intervals in single variable calculus is played by convex sets in multivariable calculus. For example, here is a multivariable generalization of \cref{constant-iff-derivative-zero-single}. We will use the single variable version to prove this multivariable version. 
...
\begin{proof}
	We'll prove this only when $|-|$ denotes the max norm. The proof for the euclidean norm is a bit trickier: see \cite[theorem 9.19]{rudin}. Suppose first that $n = 1$. If $a = b$, there is nothing to do, so we can assume that $a \neq b$. Let \[ h = \frac{b-a}{|b-a|}, \]
...
Since $\sigma$ is a continuous function on the compact interval $[a_1, a_2]$, the extreme value theorem guarantees that it attains its maximum at some point $\xi \in [a_1, a_2]$. Observe that $\sigma(a_1) = \sigma(a_2) = 0$ and that $\sigma \geq 0$. If the maximum $\xi$ occurred at one of the endpoints, then $\sigma = 0$ constantly, so $f$ is constantly equal to $b$, which means the derivative vanishes on $(a_1, a_2)$ by \cref{constant-iff-derivative-zero}, contradicting our assumption that $f$ is \'etale. Thus $\xi$ is an interior point of the interval $(a_1, a_2)$. Then the interior extremum theorem \ref{interior-extremum} plus the chain rule tells us that
\[ 0 = \sigma'(\xi) = 2f'(\xi),  \]
which means that $f'(\xi) = 0$, again contradicting our assumption that $f$ is \'etale. 
