...
As you may be able to tell, we'll be using linear algebra heavily in this chapter. If you need a brief refresher, check out \cref{linear-algebra}. We'll also need some facts about the operator norm; I encourage you to look at \cref{operator-norm-definition} and at least skim through \cref{operator-norm-basic}. 
...
\begin{exercise}
	Do the exercises from \cref{prerequisites} that are invoked in the proof above (\cref{less-than-small-is-small,little-o-vector-space,norm-is-hausdorff}).
\end{exercise}
...
\begin{proof}
	...
	Since $dg_{f(a)}$ is linear and $|r(v)| = o(|v|)$, \cref{chain-rule-details} part (\ref{linear-of-small-is-small}) guarantees that $|dg_{f(a)}(r(v))| = o(|h|)$ also. Thus, by \cref{little-o-vector-space}, it is sufficient to prove that $|s(df_a(v) + r(v))| = o(|v|)$. To do this, define $\eta(w) = |s(w)|/|w|$ and then notice that 
...
\begin{exercise} \label{chain-rule-details}
	...
	\begin{hint}[for part (\ref{linear-of-small-is-small})] Even though the operator norm does not appear anywhere in the statement of this exercise, you might find the concept useful. In particular, you might consider doing \cref{operator-norm-reformulation} and then using it to prove (\ref{linear-of-small-is-small}). 
	\end{hint}
\end{exercise}
...
\begin{proof}
	We can assume, without loss of generality, that $n = 1$ (cf. \cref{deriv-zero-implies-constant-multivar-details}). We want to show that, for any $a,  a' \in U$, we have $f(a) = f(a')$. Consider the function $\gamma : [0,1] \to B$ given by
	\[ \gamma(t) = ta' + (1-t)a.  \]
	In other words, $\gamma$ is the ``straight line path'' starting at $a$ and ending at $a'$. Notice that $\gamma(t) \in B$ for all $t$ because $a$ and $a'$ are both inside $B$ and $B$ is an open ball. Also, $\gamma$ is differentiable (cf. \cref{deriv-zero-implies-constant-multivar-details}). Thus $f \circ \gamma$ is a differentiable function $[0,1] \to \R$ and 
	\[ (f \circ \gamma)'(t) = d(f \circ \gamma)_t(1) = df_{\gamma(t)}( d\gamma_{t}(1) ) = 0 \]
	...
	\[ f(a) = f(\gamma(0)) = f(\gamma(1)) = f(a'). \qedhere \]
\end{proof}
...
\begin{definition}
	A function $f : U \to \R^n$ is \emph{continuously differentiable} if $\partial_i f_j : U \to \R$ exists and is continuous for all $i = 1, \dotsc, m$ and $j = 1, \dotsc n$. Such a function is automatically differentiable by \cref{continuous-partials}. 
\end{definition}

Continuously differentiable functions have lots of nice properties. 
...
\begin{theorem}[Inverse function theorem] \label{inverse-function-theorem}
	Suppose $f : U \to \R^n$ is continuously differentiable and \'etale at a point $a \in U$. Then there exists an open neighborhood $V$ of $a$ inside $U$ such that $f(V)$ is an  open subset of $\R^n$, the restriction $f|_{V}$ of $f$ to $V$ is a bijection $V \to f(V)$, and the inverse $(f|_V)^{-1} : f(V) \to V$ is also continuously differentiable. 
\end{theorem}

The proof of \cref{inverse-function-theorem} is lengthy, challenging, and relegated to a starred section below. But here are some applications. 
...
We begin with the following multivariable version of \cref{bounded-derivative}. Again, we will actually use the single variable version to prove the multivariable version. 
...
\begin{proposition} \label{continuous-differentiable-partials}
...
\begin{proof}
	...
	Conversely, suppose that all of the partial derivatives $\partial_i f$ exist and are continuous. Then \cref{continuous-partials} guarantees that $f$ is differentiable. The final step is to show that $df : U \to \mathscr{L}(\R^m, \R)$ is continuous; This is left for you to check; see \cref{continuous-partials-details} below. 
\end{proof}
