...
The exponential, sine, and cosine functions are usually defined using power series. 
...
Here is some further practice with differentiation of transcendental functions. This exercise is the first of a series; the function $\sin(1/x)$ and variants can be used to construct a whole host of examples of bizarre functions. 
...
Another caveat to \cref{interior-extremum} is that, even if $f$ has a local extremum at a point, it need not be that the derivative ``changes sign'' at that point. This is another example of the pathological behavior that can be exhibited by functions involving $\sin(1/x)$. 
...
And again, there are pathologies involving monotonicity than can be exhibited by functions involving $\sin(1/x)$. 

\begin{exercise} \label{positive-not-monotonic}
	...
	\begin{enumerate}[(a)]
		...
		\item Show that $f$ is not monotonic on any open interval containing 0.  
	\end{enumerate}
\end{exercise}

\subsection{Darboux's theorem \texorpdfstring{$\star$}{*}} \label{darboux-section}

Darboux's theorem is not commonly discussed in elementary calculus, nor is it strictly necessary for what follows. But it is an interesting fact about derivatives, and is sometimes useful to know. Roughly, the theorem says that derivatives are ``close'' to being continuous, but in a slightly strange way. Before getting to the precise statement of the theorem, we make the following definition. 
...
For example, we saw in \cref{positive-not-monotonic} that there is a differentiable function whose derivative is positive at a point, but which is not monotonic on any interval containing that point. That cannot happen with continuously differentiable functions. 

\begin{example}
	Suppose $f : U \to \R$ is continuously differentiable, and $a \in U$ is a point such that $f'(a) > 0$. Show that there exists an open interval containing $a$ on which $f$ is strictly increasing. 
\end{example}

\subsection{\texorpdfstring{$C^k$}{Ck} functions}

Unfortunately, insisting that the derivative of a function be continuous does not rule out all possible bizarre behavior. For example, in \cref{extremum-no-sign-change}, we saw a differentiable function $f$ with an absolute minimum at a point, but the derivative does not ``change sign'' at that point. In fact, that function is also continuously differentiable (cf. \cref{extremum-no-sign-change-continued}), so merely insisting on continuous differentiability does not quite manage to rule out that kind of pathological behavior. However, as it turns out, the derivative of $f'$, called the ``second derivative'' of $f$ and denoted $f''$, turns out \emph{not} to be continuous (cf. \cref{extremum-no-sign-change-continued}). 
...
This suggests the following inductive chain of definitions. 
...
The class of smooth functions is very well-behaved. On the one hand, smoothness rules out lots of pathological behavior, like the behavior that we saw in \cref{positive-not-monotonic,extremum-no-sign-change,extremum-no-sign-change-continued} with $\sin(1/x)$-type functions. On the other hand, there is a vast array of smooth functions; we can tailor smooth functions to almost arbitrary specifications. 
...
\subsubsection*{Infinitely flat functions, bump functions, and bridging functions}
...
Next up, we'll discuss an example of a ``bridging function.'' It will be constant on $(-\infty, -1]$, and also constant on $[1, \infty)$, but the values on these two intervals is different; and on $(-1,1)$, the function ``smoothly bridges the gap'' between the two values. 
...
\begin{exercise} \label{constant-compact-zero-outside-neighborhood}
	...
	is close to having all of the properties we want, but it's not smooth; it's clearly not even continuous. Use bridging functions to modify $\chi_I$ and bridge the discontinuities. \end{hint}
\end{exercise}