...
\begin{definition} \label{single-var-derivative-definition}
	...
	exists. If this limit does exist, its value is called the \emph{derivative of $f$ at $a$}, and it is denoted either $f'(a)$ or
...
\begin{proof}[Second proof of the chain rule, continued]
	...
	Observe that $df_a + r$ is continuous at $0$ by \cref{chain-rule-single-second-proof-details} below; moreover, we have just seen that $\eta$ is continuous at $(df_a + r)(0) = 0$. Thus the composite $\eta \circ (df_a + r)$ is also continuous at 0. Thus
...
\begin{exercise} \label{chain-rule-single-second-proof-details}
...
The proofs of these facts are omitted not because the proofs are particularly difficult, but because we would have to go on a long detour to even define the functions involved sufficiently rigorously to prove anything about them. That would distract us from our object of meditation: the derivative.
...
\begin{exercise} \label{power-times-sine}
	\begin{enumerate}[(a)]
		...
		is differentiable at 0 but the derivative $f'$ is not continuous at 0.  
	\end{enumerate} 
\end{exercise}

\section{Derivative at local extrema}
...
The following is a key property of derivatives. 

\begin{exercise}[Fermat's interior extremum theorem] \label{derivative-at-extremum}
	...
	Since $f$ is differentiable at $x = a$, $\sigma$ is continuous at $h = 0$. If $a$ is a local minimum, what can you say about the values of $\sigma$ for nonzero values of $h$? What about if $a$ is a local maximum?
\end{exercise}

\section{Differentiable functions}

\begin{definition}
	If $U$ is an open subset of $\R$, a function $f : U \to \R$ is \emph{differentiable} if $f$ is differentiable at every $a \in U$. The function $U \to \R$ defined by $a \mapsto f'(a)$ is called the \emph{derivative} of $f$, and is denoted by $f'$. 
\end{definition}

\subsection{Mean value theorem}

The mean value theorem \ref{mean-value-theorem} is an important foundational result; it allows us to prove many results about derivatives which we are familiar with from calculus. Rolle's theorem \ref{rolles-theorem} is a special case of the mean value theorem, but the general case can be derived from this special case. 

\begin{exercise}[Rolle's theorem] \label{rolles-theorem}
	Suppose $a < b$ and let $f : [a, b] \to \R$ be a continuous function that is differentiable on $(a, b)$. If $f(a) = f(b) = 0$, prove that there exists $c \in (a, b)$ such that $f'(c) = 0$. \textit{Hint}. Any function must fall into at least one of the following three cases: (i) $f$ is constantly zero, (ii) $f$ takes on positive values somewhere on $[a, b]$, and (iii) $f$ takes on negative values somewhere on $[a, b]$. Deal with the first case using \cref{constant-deriv-zero}, and the latter two cases using the extreme value theorem\footnotemark\ together with \cref{derivative-at-extremum}. 
\end{exercise}

\footnotetext{The \emph{extreme value theorem} says that if $K$ is a compact metric space and $f : K \to \R$ is continuous, then $f$ achieves its global maximum and its global minimum somewhere on $K$.}

\begin{exercise}[Mean value theorem] \label{mean-value-theorem}
	Suppose $a < b$ and let $f : [a, b] \to \R$ be a continuous function that is differentiable on $(a, b)$. Prove that there exists $c \in (a, b)$ such that
...
\begin{proof}
	We need to show that, for any distinct pair of points $a, b \in I$, we have $f(a) = f(b)$. We can assume without loss of generality that $a < b$. Since $f$ is differentiable on $I$ and $[a,b] \subseteq I$, we know in particular that $f$ is continuous on $[a,b]$ and differentiable on $(a,b)$, so the mean value theorem applies: there exists a point $c \in (a,b)$ such that
...
\begin{definition} \label{cn-single-var}
	...
	\begin{itemize}
		...
		\item In general, for any positive integer $n$, we say that $f$ is $C^n$ if $f'$ exists and is $C^{n-1}$. This means that the \emph{$n$th derivative} $f^{(n)} = (f^{(n-1)})'$ exists and is continuous.
	\end{itemize}
\end{definition}

\begin{exercise} \label{sums-scalars-cn-singlevar}
	Prove that, for every non-negative integer $n$, the set of all $C^n$ functions $U \to \R$ is a vector space. 
\end{exercise}

\begin{exercise}[General Leibniz Rule] \label{product-cn-singlevar}
	Suppose that $f, g : U \to \R$ are $C^n$ functions. Prove that the product $fg$ is also $C^n$, and that
	\[ (fg)^{(n)} = \sum_{k = 0}^n {n \choose k} f^{(k)} g^{(n-k)}. \]
\end{exercise}

\begin{exercise} \label{composite-cn-singlevar}
	Show that the composite of two $C^n$ functions is also $C^n$. \textit{Hint}. Induct on $n$. The base case $n = 0$ is clear. For the inductive step, use the chain rule \ref{chain-rule-single} and the first part of \cref{product-cn-singlevar}. 
\end{exercise}

Observe that we have an infinite chain of implications
\[ \dotsb \implies C^3 \implies C^2 \implies C^1 \implies C^0. \]
The following exercise shows that all of these implications are strict; in other words, for every $n$, there exists a $C^n$ function which is not $C^{n+1}$.

\begin{exercise}
	For every non-negative integer $n$, let 
	\[ f_n(x) = x^n|x|. \]
	Prove that $f_n$ is $C^n$ but not $C^{n+1}$. \textit{Hint.} First prove that $f_{n+1}' = (n+2)f_n$. 
\end{exercise}

\subsection{Smooth functions}

\begin{definition}
	A function $f : U \to \R$ is said to be $C^\infty$, or \emph{infinitely differentiable}, or \emph{smooth}, if it is in $C^n$ for all $n$. 
\end{definition}
...
\begin{proof}
	...
	We now prove by induction that $f^{-1}$ is $C^n$ for all integers $n$. For the base case, we know from above that it is $C^0$ (in fact, we even know more: we know that $f^{-1}$ is even differentiable, but all we need to know for the base case is that it is continuous). Suppose inductively that $f^{-1}$ is $C^n$ for some non-negative integer $n$. Then, as we saw above, $(f^{-1})'$ is the composite of $f^{-1}$, $f'$, and inversion. The latter two are smooth, so in particular are $C^n$. By the inductive hypothesis, we know that $f^{-1}$ is $C^n$. Thus $(f^{-1})'$, being the composite of three $C^n$ functions, is also $C^n$ by \cref{composite-cn-singlevar}. This means that $f^{-1}$ is $C^{n+1}$, which completes the induction. 
\end{proof}
...
\begin{remark} \label{exponential-limit}
	You probably remember proving that
	\[ \lim_{u \to \infty} \frac{u^m}{e^u} = 0 \]
	for any non-negative integer $m$ using L'H\^{o}pital's rule in your calculus class. We haven't proved L'H\^{o}pital's rule (too much of a tangent (pun intended)), but we'll use this fact anyway. In particular, note that by making the substitution $u = 1/x$ above, we find that
...
\begin{exercise} \label{infinitely-flat}
	...
	\begin{enumerate}[(a)]
		...
		\[ f^{(n)}(x) = \begin{cases} p_n(x) \cdot \dfrac{e^{-1/x}}{x^{2n}} & \text{if } x > 0 \\ 0 & \text{if } x \leq 0 \end{cases} \]
		where $p_n$ is a polynomial function of degree $n-1$ such that $p_n(0) = 1$. 
	\end{enumerate}
	Thus $f$ is an example of a smooth function that is ``infinitely flat'' at $x = 0$, in the sense that $f^{(n)}(x) = 0$ for all $n$, but is not constantly equal to 0. 
\end{exercise}
...
In the following two examples, we will have to invoke the fundamental theorem of calculus, even though we have not proved it (or even defined integrals, for that matter). 
