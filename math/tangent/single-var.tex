...
\begin{caution}
	Some authors don't define what it means for a function to be differentiable on a non-open subset of $\R$ at all; some define it slightly differently than we have above. One should be cautious when talking about differentiability on a non-open subset of $\R$, because there is potential for misunderstanding. 
\end{caution}
...
The following can be proved using either the definition of the derivative \ref{single-var-derivative-definition} or the equivalent ``best linear approximation'' characterization of \cref{unique-linear-single-var}. My suggestion is that you try both methods for these exercises. 
...
When you use \cref{unique-linear-single-var} to prove the following, you may find it useful to have done \cref{little-o-vector-space}. 
...
In this section, we discuss differentiation of some common ``transcendental functions,'' but without complete proofs. The proofs are omitted because they would involve long-ish detours away from the intended object of meditation: the derivative.

First off, here is the most general statement of the power rule, but you should note that \cref{power-rule-fractional} already covers the most common and most important cases. 
...
\begin{exercise}[Interior extremum theorem] \label{interior-extremum}
	...
	\begin{hint} Consider the following function (which we've seen before, during the first proof of the chain rule in \cref{chain-rule-single-section}).
...
\begin{exercise} \label{interior-extremum-converse-false}
	Give an example of a function $f : \R \to \R$ where $f'(0) = 0$, but $f$ does not have a local extremum at 0. 
\end{exercise}
...
Here is the first of many, many consequences of the mean value theorem. 
...
\begin{proposition} \label{deriv-zero-implies-constant}
	Suppose $I$ is an interval and that $f : I \to \R$ is differentiable. If $f' = 0$, then $f$ is constant. 
\end{proposition}
...
\begin{exercise} \label{deriv-increasing-decreasing}
	Suppose $I$ is an interval and that $f : I \to \R$ is differentiable. Prove the following four statements. 
	\begin{enumerate}[(a)]
	...
	\end{enumerate} 
	\begin{hint} For all four, imitate the proof of \cref{deriv-zero-implies-constant}. \end{hint}
\end{exercise}

\subsection{Darboux's theorem \texorpdfstring{$\star$}{*}}

Darboux's theorem is not commonly discussed in elementary calculus, nor is it strictly necessary for what follows. But it is an interesting fact about derivatives, and is sometimes useful to know. Roughly, the theorem says that derivatives are ``close'' to being continuous. Before getting to the precise statement of the theorem, we make the following definition. 
...
\begin{exercise} \label{discontinuous-ivp}
	...
	is discontinuous at $x = 0$ but has the intermediate value property. \begin{hint} First show that for any non-degenerate interval $I$ containing 0, we have $f(I) = [-1,1]$. Then explain how this implies the intermediate value property. 
	\end{hint}
\end{exercise}

We know that the derivative $f'$ of a differentiable function $f$ need not be continuous (cf. \cref{power-times-sine}). Darboux's theorem asserts that, even though $f'$ might not be continuous, it must still have the intermediate value property! This means, for example, that derivatives cannot have removable or jump discontinuities, since we saw in  \cref{removable-jump-ivp} that functions with these kinds of discontinuities do not have the intermediate value property. 
...
\begin{definition} \label{cn-single-var}
	...
	\begin{itemize}
		...
		\item $f$ is $C^2$ if it is differentiable and the derivative $f'$ is $C^1$. In other words, this means that the derivative of the derivative $(f')'$, typically denoted $f''$ or $f^{(2)}$ and called the \emph{second derivative}, exists and is continuous. 
...
\begin{exercise} \label{composite-cn-singlevar}
	Show that the composite of two $C^k$ functions is also $C^k$. \begin{hint} Use induction on $k$. The base case $k = 0$ is clear. For the inductive step, use the chain rule \ref{chain-rule-single} and \cref{product-cn-singlevar}. \end{hint}
\end{exercise}
