...
You should consult a dedicated linear algebra textbook to review the definitions of vector spaces, subspaces, linear independence, span, dimension, and linear maps. You should also consult an analysis textbook to review the definitions of metric spaces, equivalence of metrics, open subsets, closed subsets, continuous maps, compactness, connectedness, etc. 
...
This also defines a norm in the sense of \cref{norm-definition}. 
...
Observe that $\mathscr{M}_{n \times m}$ is a finite dimensional vector space. In fact, $\dim \mathscr{M}_{n \times m} = mn$. If $E_{j,i}$ denotes the matrix whose $(j,i)$-entry is 1 and all other entries are 0 for $i = 1, \dotsc, m$ and $j = 1, \dotsc, n$, then the list of all of these $mn$ matrices forms a basis for $\mathscr{M}_{n \times m}$. 

We can use this basis to define a $L^2$ and max norm on $\mathscr{M}_{n \times m}$, as in \cref{l2-norm,max-norm}. Explicitly, if $A$ is a $n \times m$ matrix whose $(j,i)$-entry is $a_{j,i}$, we have the following. 
...
\begin{proof}
	Since $\det : \mathscr{M}_{n \times n} \to \R$ is continuous, and $\R \setminus \{0\}$ is an open subset of the codomain, we see that
	\[ \GL_n = \text{det}^{-1}(\R \setminus \{0\}) \]
	must be open in $\mathscr{M}_{n \times n}$. 
\end{proof}
