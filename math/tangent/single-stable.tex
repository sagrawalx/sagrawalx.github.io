...
\begin{definition}
	If $f : S \to \R$ is differentiable at an interior point $a \in S$, we define $df_a : \R \to \R$ to be the linear map $df_a(h) = f'(a)h$. The function $df_a$ is sometimes called the \emph{total derivative} or the \emph{differential of $f$ at $a$}. 
\end{definition}
...
\begin{proof}[First proof of the chain rule]
	...
	See \cref{value-difference-chain-rule}. You will verify in \cref{chain-rule-single-first-proof-details} below that $\epsilon$ is continuous at 0. 
...
\begin{definition} \label{continuously-differentiable}
...
\begin{exercise} \label{critical-dense-implies-constant}
	Suppose $f : U \to \R$ is continuously differentiable. Show that, if $f'(x) = 0$ for all $x \in U \cap \Q$, then $f$ is constant. 
\end{exercise}
