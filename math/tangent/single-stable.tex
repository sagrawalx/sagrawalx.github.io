...
Thus \cref{differential-unique-single} says that $df_a$ is the only good linear approximation to the function \[ h \mapsto f(a+h)-f(a). \] Being the \emph{only good} linear approximation, it is also the \emph{best} linear approximation, whence the name of this section. The following is an important version of this same idea that comes up frequently in proofs. 
...
\begin{exercise}[L'H\^opital's rule, weak version] \label{weak-lhopital}
	...
	\[ f(x) = f(a+h) = f(a) + df_a(h) + r(h) = f'(a)h + r(h) = \left( f'(a) + \frac{r(h)}{h} \right)h, \]
