...
\[ \frac{f(a+h)-f(a)}{h}. \]
This quantity is often called a \emph{difference quotient}\index{difference quotient}. See \cref{figure-difference-quotient} for a geometric explanation of this difference quotient. By letting $h \to 0$, we get closer and closer to what we intuitively understand to be the slope of the tangent line. 
...
\begin{definition} \label{derivative-definition-single} \index{differentiable!differentiable at a point} \index{derivative!derivative at a point}
	...
	\[ \left.\frac{df}{dx}\right|_{x = a} \quad \left.\frac{d}{dx} f(x)\right|_{x = a} \quad \frac{df}{dx} (a) \] 
\end{definition}
...
\begin{exercise}[Power rule for positive integer exponents] \label{power-rule-positive} \index{power rule}
	Prove that, if $f(x) = x^n$ for a positive integer $n$, then $f'(a) = na^{n-1}$. \begin{hint} Recall the binomial theorem, which says that
	\[ (a+h)^n = \sum_{k = 0}^n \binom{n}{k} a^k h^{n-k}. \]
	\end{hint}
\end{exercise}
...
\begin{definition} \index{differentiable} \index{derivative}
	We say that a function $f : S \to \R$ is \emph{differentiable} if $f$ is continuous on $S$ and differentiable at every interior point of $S$. Letting $S^\circ$ denote the interior of $S$, the function $S^\circ \to \R$ defined by $a \mapsto f'(a)$ is called the \emph{derivative} of $f$. We will usually denote this function by $f'$, but occasionally we will also use the notation \[ \frac{df}{dx}, \] where $x$ is the variable that is being used to denote the argument to $f$. 
	Note that \cref{differentiable-implies-continuous-single} makes the continuity on $S^\circ$ automatic. 
\end{definition}

\begin{caution}
	Other authors may define differentiability on a non-open subset slightly differently, so you should be cautious when reading other texts and you see assertions about a function on a non-open subset of $\R$ being differentiable. The definition we have given will be convenient for our purposes, but because of the potential for misunderstanding, we will apply it sparingly for non-open subsets. Specifically, we will only use this definition either when the domain is open, or when it is an interval.\footnotemark
\end{caution}

\footnotetext{By \text{interval}, we mean an uncountable connected subset of $\R$. This means that intervals can be open, closed, or half-open; and they can be bounded or unbounded.}
...
Notice that the function $h \mapsto f'(a)h$ is a linear map $\R \to \R$ (in the sense of linear algebra). One way to interpret the following result is that the function $h \mapsto f'(a)h$ is the \emph{only} linear map that is a good approximation\index{linear approximation} to $h \mapsto f(a+h)-f(a)$. The precise statement of the result uses little-oh notation; if you have not seen this notation before, see \cref{little-o-definition}.
...
\begin{remark}[``Error in approximation'' function] \label{error-in-approximation}
	Suppose $f : S \to \R$ is differentiable at an interior point $a \in S$. For sufficiently small values of $h$, we can define the ``error in approximation'' function\index{error in approximation function@``error in approximation'' function}
...
As a first example of an application of this idea, here is a weak version of l'H\^opital's rule\index{lhopitals rule@L'H\^opital's rule}. 
...
\begin{exercise}[L'H\^opital's rule, weak version] \label{weak-lhopital}
	...
	\begin{hint} If $x = a+h$, we have
	\[ f(x) = f(a+h) = f(a) + df_a(h) + r(h) = f'(a)h + r(h), \]
	where $r$ is the ``error in approximation'' function for $f$, as defined in \cref{error-in-approximation}. 
	\end{hint}
\end{exercise}

\section{Computing derivatives}

The goal in this section is to prove a few of the basic rules of differentiation that one normally encounters in an introductory calculus course. 

\subsection{Sum and scalar multiples rule}

The following can be proved using either the definition of the derivative \ref{derivative-definition-single} or the equivalent ``best linear approximation'' characterization of \cref{differential-unique-single}. My suggestion is that you try both methods for these exercises. 
...
When you use \cref{differential-unique-single} to prove the following, you may find it useful to have done \cref{little-o-vector-space}. 
...
\begin{exercise}[Power rule for integer exponents] \label{power-rule-integer} \index{power rule}
	...
	\begin{hint} Use the quotient rule for negative integers. The exponent zero is a special but easy case; deal with it separately. 
	\end{hint}
\end{exercise}
...
\begin{exercise}[Derivatives of inverses] \label{inverse-derivative} \index{inverse function}
	Suppose $f : S \to \R$ is injective, differentiable at an interior point $a \in S$, and that $b = f(a)$ is an interior point of $T$. Show that, if the inverse $f^{-1} : f(S) \to \R$ is differentiable at $b$, then 
	\[ (f^{-1})'(b) = \frac{1}{f'(f^{-1}(b))}. \]
\end{exercise}

\subsubsection*{First proof}

The first proof uses the definition of the derivative \cref{derivative-definition-single}, but there is a subtle technicality involved; so, let us discuss it informally first before diving into the formal proof.  
...
\begin{exercise} \label{chain-rule-single-first-proof-details}
	Verify the following facts used in the proof above.
	\begin{enumerate}[(a)]
		\item Prove that $\epsilon$ is continuous at 0.
		\item Prove \cref{alternative-to-multiplicative-cross-term}. 
	\end{enumerate}
...
\begin{proof}[Second proof of the chain rule] \renewcommand{\qedsymbol}{}
	By \cref{differential-unique-single}, it is sufficient to prove that
	\[ g(f(a+h))-g(f(a))-dg_{f(a)}(df_a(h)) = o(|h|) \text{ as } h \to 0. \]
	Define the following ``error in approximation'' functions, as in \cref{error-in-approximation}.\index{error in approximation function@``error in approximation'' function} 
	\[ \begin{aligned} 
	r(h) &= f(a+h)-f(a) - df_a(h) \\ 
...
\begin{definition} \index{extremum!local extremum} \index{extremum!local maximum} \index{extremum!local minimum} \index{extremum!absolute maximum} \index{extremum!absolute minimum} \index{extremum!absolute extremum}
	Supppose $X$ is a metric space and $f : X \to \R$ is a function. Suppose $a \in X$. 
	\begin{itemize}
		...
		\item $a$ is a \emph{local extremum} if it is either a local maximum or a local minimum.
		...
		\item $a$ is a \emph{absolute extremum} or just \emph{extremum}, if it is either an absolute maximum or an absolute minimum.
	\end{itemize}
\end{definition}
...
\begin{exercise}[Interior extremum theorem] \label{interior-extremum} \index{extremum!interior extremum theorem} \index{interior extremum theorem|see {extremum}}
	...
	\begin{hint} 
		...
		Since $f$ is differentiable at $x = a$, we know that $\sigma$ is continuous at $h = 0$. If $a$ is a local minimum, what can you say about the values of $\sigma$ for positive values of $h$? Negative values of $h$? Then consider the case when $a$ is a local minimum. 
	\end{hint}
\end{exercise}
...
We will prove this for \emph{rational} exponents in \cref{power-rule-fractional}. We won't prove it in full generality, because that would require a long tangential discussion of what $x^r$ even means for irrational $r$. Actually, once we have a sufficiently rigorous definition of $x^r$ for irrational $r$, it turns out that \cref{power-rule-general} is actually a relatively straightforward \emph{consequence} of \cref{power-rule-fractional}. 
...
We'll prove that power series can be differentiated term-by-term in \cref{differentiating-power-series}. This allows us to prove the following standard facts. 
\begin{enumerate}[(1)]
...
\begin{remark}
	Defining the exponential, sine, and cosine functions using power series is mathematically fine, but there are numerous facts about these functions that are not clear from the power series, and one needs to prove these facts separately. For example, it's useful knowing that, if $e := \exp(1)$, then $\exp(x) = e^x$. This fact is not immediately clear from the power series definition; it requires some proof, and we won't prove it here, but you should feel free to use it. It's obviously also useful to know that sines and cosines have something to do with trigonometry. Again, this is not clear from the power series definition; it requires some proof, and we won't prove it, but you should feel free to use it. 
\end{remark}

Let's now discuss some examples using these functions. Specifically, let's think about functions involving $\sin(1/x)$. As we will see, these functions exhibit a wild variety of pathological behavior. 
...
In this section, we'll discuss functions which are differentiable not just at a point, but on an entire interval.

\subsection{Mean value theorem}

The mean value theorem \ref{mean-value-theorem} is an extremely important foundational result; it comes up incredibly often during proofs. In this section, we'll discuss the theorem and a few of its consequences; we will see many more of its consequences later on. 
...
\begin{exercise}[Rolle's theorem] \label{rolles-theorem} \index{mean value theorem!rolles theorem@Rolle's theorem} \index{rolles theorem@Rolle's theorem|see {mean value theorem}}
	Suppose $a < b$ and let $f : [a, b] \to \R$ be a differentiable function. If $f(a) = f(b) = 0$, prove that there exists $c \in (a, b)$ such that $f'(c) = 0$. \begin{hint} Any function must fall into at least one of the following three cases: (i) $f$ is constantly zero, (ii) $f$ takes on positive values somewhere on $[a, b]$, and (iii) $f$ takes on negative values somewhere on $[a, b]$. Deal with the first case using \cref{constant-implies-derivative-zero-single}, and the latter two cases using the extreme value theorem\footnotemark\ together with \cref{interior-extremum}. 
	\end{hint}
\end{exercise}
...
\begin{exercise}[Mean value theorem] \label{mean-value-theorem} \index{mean value theorem}
	Suppose $a < b$ and let $f : [a, b] \to \R$ be a differentiable function. Prove that there exists $c \in (a, b)$ such that
	\[ f'(c) = \frac{f(b) - f(a)}{b-a}. \]
	\begin{hint} Let $\ell$ be the secant line passing through $(a, f(a))$ and $(b, f(b))$. Then consider the function $g = f - \ell$. See \cref{subtracting-the-secant}. 
...
\begin{proposition} \label{constant-iff-derivative-zero-single}
	Suppose $I$ is an interval and that $f : I \to \R$ is differentiable. Then $f' = 0$ if and only if $f$ is constant\index{constant function}. 
\end{proposition}

\begin{proof}
	We showed in \cref{constant-implies-derivative-zero-single} that $f$ being constant implies $f' = 0$. Conversely, suppose we have $a < b$ in $I$. We want to show that $f(a) = f(b)$. Since $f$ is differentiable on $I$, we can apply the mean value theorem\index{mean value theorem} on the smaller closed subinterval $[a, b]$. In other words, there exists a point $c \in (a,b)$ such that
	\[ \frac{f(b)-f(a)}{b-a} = f'(c). \]
	But $f'(c) = 0$ by assumption, so $f(a) = f(b)$. 
\end{proof}

Here is another consequence, which says roughly that functions with bounded derivative cannot grow too fast.  

\begin{exercise} \label{bounded-derivative}
	Suppose $I$ is an open interval and $f : I \to \R$ is a differentiable function such that there exists a number $M$ such that $|f'(x)| \leq M$ for all $x \in I$. Show that $|f(b) - f(a)| \leq M|b-a|$ for all $a, b \in I$. 
\end{exercise}
...
Here is an important property of strictly monotone continuous functions. 

\begin{exercise} \label{continuous-strictly-monotone-open}
	Prove that, if $I$ is an open interval and $f : I \to \R$ is continuous and strictly monotone, then $f(I)$ is also an open interval. Conclude that $f$ is an open map\index{open!open map} (ie, that the image under $f$ of any open subset of the domain is open).
	\begin{hint}
		Suppose $f$ is strictly increasing. If $I = (a, b)$, define $c = \lim_{x \to a^+} f(x)$ and $d = \lim_{x \to b^-} f(x)$ and use the intermediate value theorem to prove that $f(I) = (c, d)$. Then deal with the case when $f$ is strictly decreasing. 
	\end{hint}
\end{exercise}
...
\begin{exercise}[Derivatives and monotonicity] \label{monotone-derivative} \index{monotone}
	Suppose $I$ is an interval and that $f : I \to \R$ is differentiable. Prove the following. 
	\begin{enumerate}[(a)]
		\item $f' \geq 0$ if and only if $f$ is increasing\index{monotone!increasing}. 
		\item $f' \leq 0$ if and only if $f$ is decreasing\index{monotone!decreasing}.
	\end{enumerate}
\end{exercise}

\begin{exercise}[Derivatives and strict monotonicity] \label{strictly-monotone-derivative} \index{monotone!strictly monotone}
	Suppose $I$ is an interval and that $f : I \to \R$ is differentiable. Prove the following. 
	\begin{enumerate}[(a)]
		\item If $f' > 0$, then $f$ is strictly increasing\index{monotone!strictly increasing}. 
		\item If $f' < 0$, then $f$ is strictly decreasing\index{monotone!strictly decreasing}. 
	\end{enumerate} 
\end{exercise}

It's worth noticing that \cref{strictly-monotone-derivative} cannot be upgraded to if and only if statements. 

\begin{exercise}
	Give an example of a strictly increasing differentiable function $f : \R \to \R$ for which there exists some $a \in \R$ such that $f'(a) = 0$. 
\end{exercise}

And again, there are pathologies involving monotonicity than can be exhibited by functions involving $\sin(1/x)$. Here is an example of a function whose derivative is positive at a single point, but it is not monotone near that point. 

\begin{exercise} \label{positive-not-monotone}
	Let $f : \R \to \R$ denote the function
	\[ f(x) = \begin{cases} x + 2x^2 \sin(1/x) & \text{if } x > 0 \\ 0 & \text{if } x = 0. \end{cases} \]
	\begin{enumerate}[(a)]
		\item Show that $f$ is differentiable and calculate $f'$. In particular, show that $f'(0) = 1$. 
		\item Show that $f$ is not monotone\index{monotone} on any open interval containing 0.  
	\end{enumerate}
\end{exercise}

\subsection{Darboux's theorem \starred} \label{darboux-section}

Derivatives are often ``close'' to being continuous, but in a weird way. They cannot have ``simple'' discontinuities, which means that when they are discontinuous at all, they are discontinuous in wild ways. Darboux's theorem is one way in which this principle manifests itself. Before getting to the precise statement, we make the following definition. 
...
\begin{proof}
	...
	There are two cases two consider: either (i) $f'(a) < m < f'(b)$ or (ii) $f'(a) > m > f'(b)$. Suppose we are in case (i). By the extreme value theorem\index{extremum!extreme value theorem}, $g$ attains its minimum on the compact interval $[a,b]$ at some point $c \in [a,b]$. We claim that $c$ must be an interior point of $[a,b]$, and then interior extremum theorem\index{extremum!interior extremum theorem} \ref{interior-extremum} will imply that $g'(c) = 0$. 
...
\begin{exercise} \label{nonzero-derivative-implies-strictly-monotone}
	Suppose $I$ is an open interval and $f : I \to \R$ is a differentiable function such that $f'(x) \neq 0$ for all $x \in I$. Show that $f$ is strictly monotone.\index{monotone!strictly monotone} 
\end{exercise}

\subsection{Inverse function theorem}

\begin{theorem} \label{inverse-function-theorem-single}
	Suppose that $I$ is an open interval, that $f : I \to \R$ is differentiable, and that $f'(x) \neq 0$ for all $x \in I$. Then $f$ is open, the inverse function $f^{-1} : f(I) \to \R$ is differentiable, and 
	\[ f^{-1}(y) = \frac{1}{f'(f^{-1}(y))} \]
	for all $y \in f(I)$. 
\end{theorem}

\begin{proof}
	\Cref{nonzero-derivative-implies-strictly-monotone} tells us that $f$ is strictly monotone, so \cref{continuous-strictly-monotone-open} tells us that $f$ is an open map and that $f(I)$ is an open interval. In particular, this means that $f^{-1}$ is continuous. To see that it is differentiable, fix $y \in f(I)$ and let $x = f^{-1}(y)$. For each $k$, define \[ h(k) = f^{-1}(y + k) - f^{-1}(y). \]
	Rearranging, this is equivalent to all of the following. 
	...
	Since $f^{-1}$ is continuous at $y$, we have $\lim_{k \to 0} h(k) = 0$. Thus the denominator on the far right above tends to $f'(x) = f'(f^{-1}(y))$ as $k \to 0$. Since the limit of the denominator is nonzero by assumption, the result follows from the quotient rule for limits. 
\end{proof}
...
Derivatives interact somewhat strangely with limits. Here are some examples to illustrate that the limit of the derivatives is necessarily the derivative of the limit.
...
In fact, to get well-behaved interacted between limits and derivatives, what we really need is uniform convergence \emph{of the derivatives}. 

\begin{theorem} \label{differentiation-uniform-limit}
	Suppose $U$ is an open subset and $f_n : U \to \R$ is a differentiable function for each $n = 0, \dotsc$ such that $\lim f_n = f$ pointwise. If the derivatives $f_n'$ converge uniformly on compact subsets of $U$, then $f$ is differentiable and $\lim f_n' = f'$. 
\end{theorem}
...
\begin{proof}
	...
	In fact, $\sigma_n$ converges \emph{uniformly} to $\sigma$ on a compact interval $I$ containing 0. To see this, observe that for any $m, n$, and $h \in I \setminus \{0\}$, we have
	\[ \sigma_m(h) - \sigma_n(h) = \frac{(f_m(a+h) - f_n(a+h)) - (f_m(a) - f_n(a))}{h} = f_m'(\xi) - f_n'(\xi), \]
	where for the last step we have applied the mean value theorem to the function $f_m - f_n$ on the closed interval between $a$ and $a + h$ to find $\xi$ strictly between $a$ and $a + h$. Moreover, we have $\sigma_m(0) - \sigma_n(0) = f_m'(a) - f_n'(a)$. In other words, for every $h \in I$, there exists $\xi \in a + I$ such that $\sigma_m(h) - \sigma_n(h) \leq f_m'(\xi) - f_n'(\xi)$. Thus
	\[ \|\sigma_m - \sigma_n\|_{\spn, I} \leq \|f_m' - f_n'\|_{\spn, a+I}. \]
	Since the derivatives converge uniformly on the compact subset $a+I$, they are uniformly Cauchy. It follows from the above inequality that the $\sigma_n$ are also uniformly Cauchy, and therefore uniformly convergent. But $\sigma$ is the pointwise limit of the $\sigma_n$, so it must be the uniform limit. 
	
	Since the $\sigma_n$ are continuous on $I$, and uniform limits of continuous functions are also continuous, we conclude that $\sigma$ is also continuous. This proves that $f$ is differentiable at $a$ and $f'(a) = g(a) = \lim f_n'(a)$. 
\end{proof}
...
\begin{proof}
	...
	\[ \sum_{n = 0} na_n x^{n-1} \]
	is also $R$. Let $R'$ denote the radius of convergence of this power series; we want to show that $R' = R$. Observe that
	\[ \frac{1}{R'} = \limsup_{n \to \infty} (na_n)^{1/(n-1)} = \limsup_{n \to \infty} ((na_n)^{1/n})^{n/(n-1)}. \]
	We know that $\lim n^{1/n} = 1$ and that $\lim n/(n-1) = 1$. We also know that
	\[ \frac{1}{R} = \limsup_{n \to \infty} a_n^{1/n}.  \]
	Putting all of this together, we conclude that $R = R'$. 
\end{proof}
...
\begin{comment}
\subsection{Bizarre differentiable functions \starred}
...
\begin{definition} \label{continuously-differentiable-singlevar}
...
For example, we saw in \cref{positive-not-monotone} that there is a differentiable function whose derivative is positive at a point, but which is not monotone on any interval containing that point. This is rather bizarre behavior, and it cannot happen with continuously differentiable functions. 

\begin{exercise} \label{nonzero-derivative-at-point-implies-monotone-in-neighborhood}
	Suppose $f : U \to \R$ is continuously differentiable, and $a \in U$ is a point such that $f'(a) \neq 0$. Show that there exists an open interval containing $a$ on which $f$ is strictly monotone. 
\end{exercise}

To appreciate the following fact, it's worth noting the following: for any open interval $I$, there exist differentiable functions $f : I \to \R$ such that $f'(x) = 0$ for all $x \in I \cap \Q$, but $f$ is \emph{not} constant (and $f'$ does \emph{not} constantly vanish)! These extremely bizarre functions were first found by Pompeiu \cite{pompeiu}. This pathological behavior is also ruled out by insisting that the derivative be continuous. 

\begin{exercise} \label{critical-locus-closed-singlevar}
	Suppose $f : U \to \R$ is continuously differentiable. 
	\begin{enumerate}[(a)]
		\item The \emph{critical locus}\index{critical point!critical locus} of $f$ is the set
		\[ Z = \{ x \in U : f'(x) = 0 \}. \]
		Show that $Z$ is a closed subset of $U$.
		\item Conclude that, if $f'(x) = 0$ for all $x \in U \cap \Q$, then $f'(x) = 0$ for all $x \in U$.   
	\end{enumerate}
\end{exercise}

Unfortunately, insisting that the derivative of a function be continuous does not rule out all possible bizarre behavior. For example, in \cref{extremum-no-sign-change}, we saw a differentiable function $f$ with an absolute minimum at a point, but the derivative doesn't just simply ``change sign'' at that point. In fact, it turns out that function is also continuously differentiable (cf. \cref{extremum-no-sign-change-continued}), so merely insisting on continuous differentiability does not quite manage to rule out that kind of pathological behavior. However, as it turns out, the derivative of $f'$ (called the ``second derivative'' of $f$, and denoted $f''$), turns out \emph{not} to be continuous. 

\begin{exercise} \label{extremum-no-sign-change-continued}
	Let $f : \R \to \R$ be the function from \cref{extremum-no-sign-change}. Show that $f'$ is differentiable, but that $f''$ is not continuous. 
\end{exercise}
...
\begin{exercise} \label{ck-stable-sum-scalar-single}
	Prove that the set of all $k$ times differentiable functions $U \to \R$ is a vector space. 
\end{exercise}
...
\begin{exercise} \label{ck-stable-composite-single}
	Show that the composite of two $C^k$ functions is also $C^k$. \begin{hint} Induct on $k$. The base case $k = 0$ is clear. For the inductive step, use the chain rule \ref{chain-rule-single} and \cref{ck-stable-product-single}. \end{hint}
\end{exercise}
...
\begin{lemma} \label{ck-stable-inversion-single}
	Suppose $I$ is an open interval, $f : I \to \R$ is $C^k$ injection for some $k \geq 1$, and that $f'(x) \neq 0$ for all $x \in I$. Then $f^{-1} : f(I) \to U$ is also $C^k$. \index{inverse function}
\end{lemma}
...
\begin{exercise}
	...
	\[ f_k(x) = x^k|x|. \]
	Prove that $f_k$ is $C^k$ but not $C^{k+1}$. \begin{hint} First prove that $f_{k+1}' = (k+2)f_k$. \end{hint}
\end{exercise}

\subsection{Taylor's theorem \starred}

\begin{theorem}[Taylor] \label{taylor-single}
	Suppose $f : U \to \R$ is $C^k$ for some positive integer $k$ and $a \in U$. Then there exists a unique polynomial $p_k$ at most $k$, called the \emph{degree $k$ Taylor polynomial of $f$ at $a$}, such that $|f(a+h) - p_k(h)| = o(|h|^k)$ as $h \to 0$. Moreover, we have
...
\begin{proof}
	Let $p_k(h)$ be defined as \cref{taylor-polynomial-single} and set $r(h) = f(a+h) - p_k(h)$. Observe that $r$ is $C^k$ by \cref{ck-stable-sum-scalar-single}. By direct calculation, we have $r^{(i)}(0) = 0$ for all $i = 0, 1, \dotsc, k$. 
	...
	\[ |p_k(h) - q(h)| = |p_k(h) - f(a+h) + f(a+h) - q(h)| \leq |f(a+h) - p_k(h)| + |f(a+h) - q(h)| \] 
...
We can now return to our discussion of the pathology exhibited by \cref{extremum-no-sign-change,extremum-no-sign-change-continued}, and prove that such pathologies cannot occur under some hypotheses. We prepare for this discussion with the following lemma. 

\begin{lemma} \label{eventual-nonvanishing-derivative}
	Suppose $f : U \to \R$ is $C^k$ for some $k \geq 1$, that 
	\[ f(a) = f'(a) = f^{(2)}(a) = \dotsb = f^{(k-1)}(a) = 0, \] and $f^{(k)}(a) > 0$. Then there exists an $\epsilon > 0$ such that $f$ is positive on $(a, a+\epsilon)$, and on $(a-\epsilon, a)$, $f$ is positive if $k$ is even and negative if $k$ is odd. If $f^{(k)}(a) < 0$, then all instances of ``positive'' in the conclusion get replaced by ``negative'' and vice versa. 
\end{lemma}
...
\begin{proof}
	...
	Since $f^{(k)}(a) > 0$, we also have $f^{(k)}(a)/k! > 0$. Since $\lim_{h \to 0} |r(h)/h^k| = 0$, there exists $\epsilon > 0$ such that $|r(h)/h^k| < \epsilon$ for all $|h| < \epsilon$. Then, if $0 < |h| < \epsilon$, we have
...
\begin{exercise} \label{extremum-sign-change}
	Suppose $f$ is $C^k$ for some $k \geq 1$, that
	\[ f'(a) = f^{(2)}(a) = \dotsb = f^{(k-1)}(a) = 0 \] and $f^{(k)}(a) \neq 0$. Then there exists an $\epsilon > 0$ such that $f$ is strictly monotone on $(a-\epsilon, a)$ and on $(a, a+\epsilon)$. 
	\begin{hint}
		Apply \cref{eventual-nonvanishing-derivative} to $f'$. Then use \cref{strictly-monotone-derivative}. 
	\end{hint}
\end{exercise}

\begin{example} \label{extremum-no-sign-change-final}
	Let $f : \R \to \R$ be the function of \cref{extremum-no-sign-change,extremum-no-sign-change-continued}. Then $f$ is twice differentiable and $f'(0) = f''(0) = 0$, but it is not thrice differentiable at 0. In other words, it doesn't eventually have a nonzero derivative at 0, so we're not able to apply \cref{extremum-sign-change}. 
\end{example}
...
\begin{exercise} \label{infinitely-flat}
	...
	\begin{enumerate}[(a)]
		\item Use \cref{exponential-limit} to show, using the definition of the derivative \cref{derivative-definition-single}, that $f'(0) = 0$.
		\item Check that
...
\begin{figure}
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	\caption{The graph of a function that is ``infinitely flat'' at 0.}  \label{infinitely-flat-graph}
\end{figure}

\subsubsection*{Bump functions}

Next up, we have ``bump functions.''\index{bump function} Recall that the ``support''\index{support} of a function $f : U \to \R$ is the closure of the set $\{ x \in U : f(x) \neq 0 \}$. A \emph{bump function} is a smooth function with compact support. 
...
\begin{exercise}
	Let $F$ be a closed subset of $\R$. Construct a smooth function which is zero on $F$ and nonzero outside of $F$. \begin{hint} Recall that the complement of $F$ is a countable disjoint union of open intervals (cf. \cite[theorem 6.17]{protter-morrey}). \end{hint}
\end{exercise}
