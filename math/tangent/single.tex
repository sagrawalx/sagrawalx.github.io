...
Occasionally, it's useful to talk about differentiability from just one side. 

\begin{definition}[One-sided differentiability]
	Suppose $f : S \to \R$ is a function and $a \in S$ is an interior point. Then $f$ is \emph{left differentiable at $a$} if 
	\[ \lim_{h \to 0^-} \frac{f(a+h)-f(a)}{h} \]
...
\begin{exercise}
	Suppose $r \neq s$ are two distinct real numbers and let $f : \R \to \R$ be the function
	\[ f(x) = \begin{cases} rx & \text{if } x \geq 0 \\ sx & \text{if } x < 0 \end{cases}. \]
	Show that $f$ is not differentiable at $0$, but that it is both left differentiable and right differentiable at 0. 
\end{exercise}

It's also useful to talk about differentiability not just at a single point, but everywhere on the domain. 

\begin{definition} \index{differentiable} \index{derivative}
	A function $f : S \to \R$ is \emph{differentiable} if it is continuous, and it is differentiable at every interior point of $S$. Letting $S^\circ$ denote the interior of $S$, the function $S^\circ \to \R$ defined by $a \mapsto f'(a)$ is called the \emph{derivative} of $f$. We will usually denote this function by $f'$, but will sometimes also use one of the following, where $x$ is the variable that is being used to denote the argument to $f$. \[ \frac{df}{dx} \quad \frac{d}{dx} f(x) \] 
\end{definition}

\begin{caution}
	Some authors define differentiability on a non-open subset slightly differently, so you should be cautious when you're reading other texts and see assertions about a function on a non-open subset of $\R$ being ``differentiable.'' Because of the potential for misunderstanding, we'll apply the above definition sparingly for non-open subsets. Specifically, we will only use this definition either when $S$ is open, or when it is an interval.\footnote{By \emph{interval}\index{interval}, we mean an uncountable connected subset of $\R$. This means that intervals can be open, closed, or half-open; and they can be bounded or unbounded.}
\end{caution}
...
\begin{definition}
	If $f : S \to \R$ is differentiable at an interior point $a \in U$, we define $df_a : \R \to \R$ to be the linear map $df_a(h) = f'(a)h$. The function $df_a$ is sometimes called the \emph{total derivative} or the \emph{differential of $f$ at $a$}. 
\end{definition}

Thus \cref{differential-unique-single} says that $df_a$ is the only good linear approximation to the function \[ h \mapsto f(a+h)-f(a). \] Being the \emph{only good} linear approximation, it is also the \emph{best} linear approximation, whence the name of this section.

The following is an important version of this same idea that comes up frequently in proofs. 

\begin{remark}[``Error in approximation'' function] \label{error-in-approximation}
	Suppose $f : S \to \R$ is differentiable at an interior point $a \in S$. For sufficiently small values of $h$, we can define a ``error in approximation'' function\index{error in approximation function@``error in approximation'' function} $r$ by
	...
	This equation just says that $f(a+h)$ is the sum of the value of the tangent line (which is $f(a) + df_a(h)$) and the error (which is $r(h)$). Notice that $r(0) = 0$. Moreover, $f$ is differentiable hence continuous at $a$ (cf. \cref{differentiable-implies-continuous-single}), so $r$ is also  continuous at 0. Finally, $r$ is ``small.'' More precisely, we know from \cref{differential-unique-single} that $|r(h)| = o(|h|)$ as $h \to 0$, which means that
...
\begin{exercise}[Derivatives of inverses] \label{inverse-derivative} \index{inverse function}
	Suppose $f : S \to \R$ is injective, differentiable at an interior point $a \in S$, and that $b = f(a)$ is an interior point of $f(S)$. Show that, if the inverse $f^{-1} : f(S) \to \R$ is differentiable at $b$, then 
...
The idea involved is again a clever rewriting of difference quotients. We introduce a cross term, which in this case is $f(a+h) - f(a)$. But this time we introduce this cross term ``multiplicatively'' rather than ``additively.'' More precsiely, we have
...
\begin{proof}[First proof of the chain rule]
	Consider the function $\epsilon$ defined for small values of $h$ by \[ \epsilon(h) = f(a+h)-f(a). \]
	See \cref{value-difference-chain-rule}. You will verify in \cref{chain-rule-details} below that $\epsilon$ is continuous at 0. 
	...
	You will verify in \cref{chain-rule-details} below that
...
\begin{proof}[Second proof of the chain rule, continued]
	Define $\eta(k) = s(k)/k$ for nonzero $k$. If we set $\eta(0) = 0$, the fact that $s(k) = o(|k|)$ as $k \to 0$ implies that $\eta$ is continuous at 0. Then $s(k) = \eta(k)k$ for all $k$. Applying this with $k = df_a(h) + r(h)$, we see that
...
We'll prove that power series can be differentiated term-by-term in \cref{differentiating-power-series}. This yields the following standard facts. 
\begin{enumerate}[(1)]
...
\begin{remark}
	Defining the exponential, sine, and cosine functions using power series is mathematically sound, but there are numerous facts about these functions that are not clear from the power series, and one needs to prove these facts separately. For example, it's useful knowing that, if $e := \exp(1)$, then $\exp(x) = e^x$. This fact is not immediately clear from the power series definition; it requires some proof, and we won't prove it here, but you should feel free to use it. It's obviously also useful to know that sines and cosines have something to do with trigonometry. Again, this is not clear from the power series definition; it requires some proof, and we won't prove it, but you should feel free to use it. 
\end{remark}

\subsubsection*{Examples}

Let's now discuss some examples using these functions. 

\begin{exercise}
	Let $f, g : (0,\infty) \to \R$ be the functions given by 
	\[ f(x) = \frac{\sin(x)}{x} \quad \text{and} \quad g(x) = \frac{\sin(x^2)}{x}. \]
	Show that $f$ and $g$ are both differentiable, and that 
	\[ \lim_{x \to \infty} f(x) = 0 = \lim_{x \to \infty} g(x). \] Then show that 
	\[ \lim_{x \to \infty} f'(x) = 0 \quad \text{but} \quad \lim_{x \to \infty} g'(x) \text{ does not exist}. \]
\end{exercise}

The following two examples both involve the expression $\sin(1/x)$. As we will see, functions involving this expression exhibit a wild variety of interesting pathological behavior. 
...
In this section, we'll discuss some properties of functions which are differentiable along an entire interval.
...
\begin{exercise} \label{bounded-derivative}
	Suppose $I$ is an open interval and $f : I \to \R$ is a differentiable function such that there exists a number $M$ such that $|f'(x)| \leq M$ for all $x \in I$. Show that $|f(b) - f(a)| \leq M|b-a|$ for all $a, b \in I$.\footnotemark
	\footnotetext{Using the vocabulary of \cref{sup-norm}, this result can be rephrased by saying that, if $f'$ is bounded, then $|f(b) - f(a)| \leq \|f'\|_\spn |b-a|$.}
\end{exercise}
...
Here are some important properties of strictly monotone continuous functions that you may recognize from an earlier analysis course. 
...
\begin{proof}
	\Cref{nonzero-derivative-implies-strictly-monotone} tells us that $f$ is strictly monotone, so \cref{strictly-monotone-implies-open} tells us that $f$ is an open map and that $f(I)$ is an open interval. In particular, this means that $f^{-1}$ is continuous. To see that it is differentiable, fix $y \in f(I)$ and let $x = f^{-1}(y)$. For each $k$, define \[ h(k) = f^{-1}(y + k) - f^{-1}(y). \]
...
\begin{theorem} \label{differentiation-uniform-limit}
	Suppose $U$ is an open subset and $f_n : U \to \R$ is a differentiable function for each $n = 0, 1, \dotsc$ such that $\lim f_n = f$ pointwise. If the derivatives $f_n'$ converge uniformly on compact subsets of $U$, then $f$ is differentiable and $\lim f_n' = f'$. 
\end{theorem}
...
For example, we saw in \cref{positive-not-monotone} that $f(x) = x + 2x^2\sin(1/x)$ defined a differentiable function $f : \R \to \R$ such that $f'(0) = 0$, but which is not monotone on any interval containing that point. This is rather bizarre behavior, and it cannot happen with continuously differentiable functions. 
...
We can now return to our discussion of the pathology exhibited by the function $f(x) = x^4 \left( 2 + \sin(1/x) \right)$ from \cref{extremum-no-sign-change,extremum-no-sign-change-continued}, and prove that such pathologies cannot occur under some hypotheses. We prepare for this discussion with the following lemma. 
...
\begin{exercise} \label{infinitely-flat}
	...
	\end{figure}
	\begin{enumerate}[(a)]
		\item Use \cref{exponential-limit} to show that $f'(0) = 0$.
		\item Check that
