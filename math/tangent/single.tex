...
\begin{proof}[First proof of the chain rule]
	...
	\begin{figure}[t]
		\begin{center}
			...
			\end{tikzpicture}
		\end{center}
		\caption{The graph of $f$ is depicted in black. The value of the function $\epsilon$ on a small nonzero input value $h$ is the vertical distance depicted in red.}  \label{value-difference-chain-rule}
	\end{figure}
...
\begin{proof}[Second proof of the chain rule] \renewcommand{\qedsymbol}{}
	...
	We therefore want to prove that the right-hand side is $o(|h|)$. Since \[ dg_{f(a)}(r(h)) = g'(f(a)) \cdot r(h) \] is just a scalar multiple of $r$, and $r = o(|h|)$, we see that $dg_{f(a)}(r) = o(|h|)$ also. So it is sufficient to prove that
	\[ s(df_a(h)+r(h)) = o(|h|). \]
...
\begin{proof}[Second proof of the chain rule, continued]
	...
	where we have used the fact that $|r(h)| = o(|h|)$ to see that the parenthetical expression has a finite limit (namely, $|f'(a)|$). So, by the squeeze theorem, we can conclude that \[ |s(df_a(h)+r(h))| = o(|h|), \]
...
\begin{definition} \index{differentiable} \index{derivative}
	Suppose $U$ is an open subset of $\R$. A function $f : U \to \R$ is \emph{differentiable} it is differentiable at every point in $U$. More generally, if $S$ is any subset of $\R$ and $f : S \to \R$ is a function, we say that $f$ is \emph{differentiable on the interior} if $f|_{S^\circ}$ is differentiable (ie, if $f$ is differentiable at every interior point of $S$). 
\end{definition}

Every open subset of $\R$ is a countable disjoint union of open intervals\footnotemark\ (cf. \cite[theorem 6.17]{protter-morrey}). This means that the most important case is when the domain happens to be an interval, and many of the theorems in this section will be about this situation. 
...
\begin{exercise}[Mean value theorem] \label{mean-value-theorem} \index{mean value theorem}
	Suppose $a < b$ and let $f : [a, b] \to \R$ be continuous and differentiable on the interior. Prove that there exists $c \in (a, b)$ such that
...
\begin{theorem}[Darboux] \label{darboux} \index{darbouxs theorem@Darboux's theorem}
	Suppose $I$ is an interval and $f : I \to \R$ is continuous and differentiable on the interior. Then $f'$ has the intermediate value property.\index{intermediate value property} 
\end{theorem}
...
\begin{proof}
	...
	Consider the differentiable function $g : I \to \R$ defined by $g(x) = f(x) - mx$. See \cref{subtracting-a-line}. Then $g$ is also continuous and differentiable on the interior, with $g'(x) = f'(x) - m$. Thus $f'(c) = m$ if and only if $g'(c) = 0$. Thus we would like to show that there exists $c \in (a, b)$ such that $g'(c) = 0$. 
