...
\begin{remark}[``Remainder'' function] \label{remainder-linear}
	Suppose $f : S \to \R$ is differentiable at an interior point $a \in S$. For sufficiently small values of $h$, we can define a ``remainder'' function\index{remainder} $r$ by
	...
	\end{figure}
	Notice that the value of the tangent line at $a+h$ is $f(a) + df_a(h)$, so $r(h)$ is defined to be ``what's left over'' after approximating $f$ by its tangent line (whence the name, ``remainder''). Notice that $r(0) = 0$. Moreover, $f$ is differentiable hence continuous at $a$ (cf. \cref{differentiable-implies-continuous-single}), so $r$ is also  continuous at 0. Finally, $r$ is ``small.'' More precisely, we know from \cref{differential-unique-single} that $|r(h)| = o(|h|)$ as $h \to 0$, which means that
...
\begin{proof}[Second proof of the chain rule] \renewcommand{\qedsymbol}{}
	...
	Define the following remainder functions, as in \cref{remainder-linear}.
	\[ \begin{aligned} 
	r(h) &= f(a+h)-f(a) - df_a(h) \\ 
...
The intermediate value theorem, mean value theorem, and Darboux's theorem all say similar but different things. I encourage you to ensure that you understand how the statements differ. See \cref{ivt-mvt-darboux}.

\begin{table}
	\begin{center}
	{\tabulinesep=1.5mm
	\begin{tabu} to \textwidth {| X[l] | X[c] X[c] X[c] |} 
		\hline
		& Intermediate value theorem & Mean value theorem & Darboux's theorem \\ \hline
		If $f : [a, b] \to \R$ is... & continuous & continuous and differentiable on the interior & differentiable on an open neighborhood of $[a,b]$ \\ 
		and... & $y$ is strictly in between $f(a)$ and $f(b)$ & & $m$ is strictly in between $f'(a)$ and $f'(b)$ \\
		then there exists $c \in (a,b)$ such that & $f(c) = y$. & $f'(c) = \dfrac{f(b) - f(a)}{b-a}$. & $f'(c) = m$. \\ \hline
	\end{tabu}}
	\end{center}
