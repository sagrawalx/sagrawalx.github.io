...
\begin{definition} \label{differentiable-open-domain-single} \index{differentiable} \index{derivative}
	Suppose $U$ is an open subset of $\R$. A function $f : U \to \R$ is \emph{differentiable} it is differentiable at every point in $U$. The function $U \to \R$ given by $x \mapsto f'(x)$ is called the \emph{derivative} of $f$, and is denoted $f'$. 
\end{definition}
...
\begin{exercise} \label{continuous-and-differentiable-except-at-point}
	Suppose $I$ is an open interval and $a \in I$. Let $f : I \to \R$ be continuous, and suppose further that it is differentiable at every $x \in I$ except possibly at $a$. If \[ \lim_{x \to a} f'(x) = L \]
	exists, show that $f$ is also differentiable at $a$ and that $f'(a) = L$. 
\end{exercise}
...
\begin{definition} \label{concave-up-definition}
	...
	for all $x < y < z$ in $I$. 
	See \cref{concave-up-picture}. Similarly, we say that $f$ is \emph{concave down}, or just \emph{concave}, if
	\[ \frac{f(y)-f(x)}{y-x} \geq \frac{f(z)-f(y)}{z-y} \]
	for all $x < y < z$ in $I$. 
	Notice that $f$ is concave down if and only if $-f$ is concave up. 
\end{definition}
...
\begin{exercise}
	Suppose $f : [0, \infty) \to \R$ is continuous, concave up, and that $f(0) = 0$. Show that the function $g : (0, \infty) \to \R$ defined by 
	\[ g(x) = \frac{f(x)}{x} \]
	is increasing. 
\end{exercise}

Here is the relationship between differentiation and concavity. 

\begin{exercise}[Derivatives and concavity] \label{concavity-derivative}
	Suppose $I$ is an open interval and $f : I \to \R$ is differentiable. 
	\begin{enumerate}[(a)]
		\item Prove that $f$ is concave up if and only if $f'$ is increasing. 
		\item Prove that $f$ is concave down if and only if $f'$ is decreasing. 
	\end{enumerate}
	\begin{hint}
		If $f'$ is increasing, apply the mean value theorem \ref{mean-value-theorem} to prove that $f$ is concave up. Conversely, if $f$ is concave, suppose we have $a < b$ in $I$. Let $x$ and $y$ be such that $a < x < y < b$. Show that 
