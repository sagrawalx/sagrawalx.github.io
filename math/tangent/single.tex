...
Here is another consequence, which says roughly that functions with bounded derivative cannot change too fast.  
...
To appreciate the following fact, it's worth noting the following: for any open interval $I$, there exist differentiable functions $f : I \to \R$ such that $f'(x) = 0$ for all $x \in I \cap \Q$, but $f$ is \emph{not} constant! These extremely bizarre functions were first found by Pompeiu \cite{pompeiu}. This pathological behavior is also ruled out by insisting that the derivative be continuous. 
...
\begin{exercise} \label{critical-locus-closed-singlevar}
	...
	\begin{enumerate}[(a)]
		...
		\item Conclude that, if $f'(x) = 0$ for all $x \in U \cap \Q$, then $f$ is constant.   
	\end{enumerate}
\end{exercise}
