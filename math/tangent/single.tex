...
\begin{definition} \index{differentiable} \index{derivative}
	A function $f : S \to \R$ is \emph{differentiable} if it is continuous, and it is differentiable at every interior point of $S$. Letting $S^\circ$ denote the interior of $S$, the function $f' : S^\circ \to \R$ defined by $a \mapsto f'(a)$ is called the \emph{derivative} of $f$. Sometimes also use one of the following notations in place of $f'$, where $x$ is the variable that is being used to denote the argument to $f$. \[ \frac{df}{dx} \quad \frac{d}{dx} f(x) \] 
\end{definition}
