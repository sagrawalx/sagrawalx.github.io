...
\begin{exercise}[L'H\^opital's rule, weak version] \label{weak-lhopital}
	...
	\begin{hint} First show that, if $x = a+h$ and $r$ is the ``error in approximation'' function for $f$ defined in \cref{error-in-approximation}, then
...
In fact, to get well-behaved interaction between limits and derivatives, what we really need is uniform convergence \emph{of the derivatives}, rather than of the functions themselves. 
...
\begin{proof}
	...
	If we can show that $\sigma$ is continuous at $a$, then $f$ is differentiable at $a$ and $f'(a) = g(a) = \lim f_n'(a)$, which is what we ultimately wanted to prove. To show that $\sigma$ is continuous, our strategy is to prove that $\sigma_n$ converges \emph{uniformly} to $\sigma$ on a compact interval $I$ containing 0 (not just pointwise, as we observed above). Then we can invoke the theorem which guarantees that uniform limits of continuous functions are continuous (cf. \cite[theorem 9.13]{protter-morrey}).
	...
	Since the derivatives converge uniformly on the compact subset $a+I$, they are uniformly Cauchy. It follows from the above inequality that the $\sigma_n$ are also uniformly Cauchy, and therefore uniformly convergent. Since $\sigma$ is the pointwise limit of the $\sigma_n$, it must also be the uniform limit; in other words, we have now proved that $\sigma_n$ converges uniformly to $\sigma$, as we wanted. 
\end{proof}
