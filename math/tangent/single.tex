...
\begin{exercise}
	...
	\begin{enumerate}[(a)]
		\item Show that, if $f''(a) > 0$, then $a$ is a local minimum of $f$, and that if $f''(a) < 0$, then $a$ is a local maximum of $f$. 
		\begin{hint}
			You can do this using \cref{concavity-extremums,second-derivative-concavity}. Alternatively, you can do it using Taylor's theorem \ref{taylor-single}. Try both methods. It's the Taylor's theorem method that will be more helpful for part (c) below. You may also notice that the first method really requires that $f$ be $C^2$, while the Taylor's theorem method only requires that $f$ be twice differentiable. 
		\end{hint}
		\item Give examples of three $C^2$ functions $f_1, f_2, f_3 : \R \to \R$ and a point $a \in \R$ such that
		\[ f_i'(a) = f''_i(a) = 0 \] 
		for all $i = 1, 2, 3$, where $f_1$ has a local minimum at $a$, $f_2$ has a local maximum at $a$, and $f_3$ has neither a local minimum nor a local maximum at $a$. In other words, when $f'(a) = f''(a) = 0$, the test from part (a) is inconclusive at determining whether $a$ is a local minimum, a local maximum, or neither. 
		
		\item Suppose $f$ is $C^k$ for some $k \geq 2$, that 
		\[ f'(a) = f^{(2)} = \dotsb = f^{(k-1)}(a) = 0 \]
		and that $f^{(k)}(a) \neq 0$. Formulate and prove a rule that uses the sign of $f^{(k)}(a)$ and/or the parity of $k$ to determine whether $a$ is a local minimum, a local maximum, or neither. 
	\end{enumerate}
\end{exercise}
