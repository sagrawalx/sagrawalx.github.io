...
\begin{proof}[Second proof of the chain rule] \renewcommand{\qedsymbol}{}
	...
	At this point, we've rewritten $g(f(a+h))-g(f(a))-dg_{f(a)}(df_a(h))$ in terms of $r$ and $s$. We'll now prove that the right-hand side is in fact ``small'', ie, that it is $o(|h|)$. Since \[ dg_{f(a)}(r(h)) = g'(f(a)) \cdot r(h) \] is just a scalar multiple of $r$, and $r = o(|h|)$, we see that $dg_{f(a)}(r) = o(|h|)$ also. So it is sufficient to prove that
	\[ s(df_a(h)+r(h)) = o(|h|). \]
...
Here is one application of Darboux's theorem. 
