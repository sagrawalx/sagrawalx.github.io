...
\footnotetext{The \emph{Cauchy-Schwarz inequality}\index{cauchy schwarz inequality@Cauchy-Schwarz inequality} states that
...
This norm is sometimes also called the \emph{euclidean norm} or the \emph{standard norm} on $\R^n$.\index{l2 norm@$L^2$ norm} \index{euclidean norm|see {l2 norm@$L^2$ norm}} 
...
On the other hand, we could also endow $\R^n$ with the max norm (cf. \cref{max-norm}) with respect to the standard basis. We denote this norm by $|-|_\mx$.\index{max norm} Explicitly, if $v = (h_1, \dotsc, h_n) \in \R^n$, we have
\[ |v|_\mx = \max\{ |h_1|, \dotsc, |h_n| \}. \]

\subsection{Sup norm on \texorpdfstring{$\R^n$}{Rn}-valued functions}

Let $X$ be a set and consider the set of functions $X \to \R^n$. This set is naturally a vector space under pointwise operations: if $f, g : X \to \R^n$, then
\[ (f+g)(x)= f(x) + g(x) \]
and if $\lambda \in \R$, then
\[ (\lambda f)(x) = \lambda f(x). \]

\begin{definition}
	If $X$ is a set, we define the \emph{sup norm} of a function $f : X \to \R$, denoted $\|f\|_\spn$, by
	\[ \|f\|_{\spn,X} = \sup_{x \in X} |f(x)|. \]
	If $X$ can be inferred from context, we also write simply $\|f\|_\spn$. We say that $f$ is \emph{bounded} if $\|f\|_{\spn} < \infty$. 
\end{definition}

\begin{exercise}
	Show that $\|-\|_\spn$ is a norm on the vector space of bounded\footnotemark\ functions $X \to \R^n$.
\end{exercise}
...
Observe that $\mathscr{M}_{n \times m}$ is a finite dimensional vector space. If $E_{j,i}$ denotes the matrix whose $(j,i)$-entry is 1 and all other entries are 0 for $i = 1, \dotsc, m$ and $j = 1, \dotsc, n$, then the list of all of these $mn$ matrices forms a basis for $\mathscr{M}_{n \times m}$. We can use this basis to define a $L^2$ and max norm on $\mathscr{M}_{n \times m}$, as in \cref{l2-norm,max-norm}.\index{l2 norm@$L^2$ norm} \index{max norm} Explicitly, if $A$ is a $n \times m$ matrix whose $(j,i)$-entry is $a_{j,i}$, we have the following. 
...
\begin{lemma}
	Suppose $k \leq \min\{m,n\}$ and let $Z = \{ A \in \mathscr{M}_{n \times m} : \rank(A) < k \}$. 
	Then $Z$ is a closed subset of $\mathscr{M}_{n \times m}$. 
\end{lemma}
...
\begin{definition} \index{topology} \index{topological space} \index{open!open subset}
...
\begin{definition}[Closed subsets] \index{closed}
	If $X$ is a topological space, a subset $F$ is \emph{closed} if its complement $U$ is \emph{open}. 
\end{definition}

\begin{definition}[Continuous functions] \index{continuous}
...
\begin{definition}[Subspace] \label{subspace-topology} \index{subspace} \index{subspace!subspace topology}
