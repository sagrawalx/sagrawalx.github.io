...
for all $a_1, \dotsc, a_n, b_1, \dotsc, b_n \in \R$. In other words, ``the square of the sum of the products is at most the product of the sums of the squares.'' Using the dot product, this can be phrased more succinctly as $|v \cdot w|^2 \leq |v|^2 |w|^2$ for all $v, w \in \R^n$. For a proof, see \cite[theorem 6.1]{protter-morrey}, for instance.}
...
The $L^2$ norm (cf. \cref{l2-norm}) with respect to the standard basis $e_1, \dotsc, e_n$ of $\R^n$ is the usual norm we use on $\R^n$. Explicitly, if $h = (h_1, \dotsc, h_n) \in \R^n$, we define
\[ |h| := \sqrt{h_1^2 + \dotsb + h_n^2}. \]
...
On the other hand, we could also endow $\R^n$ with the max norm (cf. \cref{max-norm}) with respect to the standard basis. We denote this norm by $|-|_\mx$.\index{max norm} Explicitly, if $h = (h_1, \dotsc, h_n) \in \R^n$, we have
\[ |h|_\mx = \max\{ |h_1|, \dotsc, |h_n| \}. \]
...
\begin{exercise} \label{operator-and-matrix-norms}
