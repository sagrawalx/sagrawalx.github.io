...
I'm currently teaching a class on algebraic geometry using Cox, Little, and O'Shea's *Ideals, Varieties, and Algorithms* {% cite clo %}. Here is a scheme-theoretic formulation of the results in section 3.3 ("Implicitization") of this book. Note that the results of {% cite clo --label section --locator 3.3 %} apply only to infinite fields, but the following works over an arbitrary base field $k$. 
...
I'm sure this is all well-known. What's more,  there's nothing new to be gained from this scheme-theoretic formulation over and above Cox-Little-O'Shea's classical formulation: since $D(g)$ is reduced, so too is any scheme-theoretic image of $D(g)$ {% stacks 056B %}. But anyway, I thought it was fun to work out, so here it is!
...
**Upshot.** Let $f : D(g) \to \mathbb{A}^n$ and $I \subseteq k[u,t,x]$ be as in the statement of the proposition above. Then $I \cap k[x]$ is the $(1+m)$th elimination ideal of $I$. The elimination theorem {% cite clo --label theorem --locator 3.1.2 %} tells us that if we compute a Gröbner basis $G$ for $I$ using a $(1+m)$th elimination order (such as the lex order where $u > t_1 > \cdots > t_m > x_1 > \cdots > x_n$), then $G \cap k[x]$ is a Gröbner basis for $I \cap k[x]$. This gives an explicit algorithm for computing the scheme-theoretic image of $f$. 
