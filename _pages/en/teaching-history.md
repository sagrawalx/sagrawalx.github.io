---
title: Teaching History
---

{% assign increment = include.increment | default: 0 %}
{% capture prefix %}{% for y in (1..increment) %}#{% endfor %}{% endcapture %}

{{ prefix }}# Primary Instructor

* UC San Diego.
    * Algebraic Geometry (Winter 2024). 
    * Probability and Statistics (Spring 2023). 
    * Cryptography (Winter 2023). 
    * Modern Algebra 1 (Fall 2022). 
* Colorado College
    * Complex Analysis (Spring 2022, Block 7). 
    * Number Theory (Spring 2022, Block 5).
    * Probability and Statistics (Spring 2022, Block J). 
    * Data Visualization (Fall 2021, Block 4). 
    * Probability and Statistics (Fall 2021, Block 1). 
    * Number Theory (Spring 2021, Block 7). 
    * Probability and Statistics (Spring 2021, Block 6). 
    * Algebraic Geometry (Fall 2020, Block 4). 
    * Number Theory (Fall 2020, Block 3). 
    * Number Theory (Fall 2020, Block 2). 
    * Linear Algebra (Spring 2020, Block 8). 
    * Probability and Statistics (Spring 2020, Block 6). 
    * Calculus 1 (Fall 2019, Block 4). 
    * Ordinary Differential Equations (Fall 2019, Block 3).
    * Real Analysis 2 (Fall 2019, Block 2).
    * Linear Algebra (Spring 2019, Block 8).
    * Calculus 1 (Spring 2019, Block 6).
    * Ordinary Differential Equations (Spring 2019, Block 5).
    * Calculus 1 (Fall 2018, Block 3).
    * Linear Algebra (Fall 2018, Block 2).
* UC Berkeley
    * Linear Algebra (Summer 2017).
    * Analysis (Summer 2015).
    * Discrete Mathematics (Summer 2014).

{{ prefix }}# Teaching Assistant

* UC San Diego
    * Linear Algebra (Fall 2023). Lead Instructor: Leesa Anzaldo. 
* UC Berkeley
    * Analytic Geometry and Calculus (Fall 2017). Instructor: Alexander Paulin.
    * Linear Algebra and Differential Equations (Spring 2016). Instructor: Xinyi Yuan.
    * Linear Algebra (Fall 2015). Instructor: Olga Holtz.
    * Discrete Mathematics (Fall 2014). Instructor: Zvezdalina Stankova.
    * Linear Algebra (Spring 2014). Instructor: Ken Ribet.
    * Calculus 1A (Fall 2013). Instructor: John Steel.
    * Linear Algebra and Differential Equations (Spring 2013). Instructor: Constantin Teleman.
    * Calculus 1A (Fall 2012). Instructor: Solobodan Simić.
    * Linear Algebra and Differential Equations (Spring 2012). Instructor: Fraydoun Rezakhanlou.
    * Calculus 1A (Fall 2011). Instructor: Ian Agol.
* UC San Diego
    * Calculus (Winter 2011). Instructor: Jiří Lebl.
    * Calculus (Fall 2010). Instructor: Jacques Verstraete.

{{ prefix }}# Grader

* UC San Diego
    * Honors Vector Calculus (Spring 2011). Instructor: John Eggers.
    * Honors Vector Calculus (Spring 2010). Instructor: John Eggers.
    * Honors Vector Calculus (Spring 2009). Instructor: Jeff Rabin.
    * Honors Multivariable Calculus (Winter 2009). Instructor: Jeff Rabin.

{{ prefix }}# Mentor

* Colorado College
    - Research project with Timothy Somerset (Summer 2021). Topic: Lifting varieties. 
    - Research project with Elizabeth Wolfe (Spring and Summer 2021). Topic: Lifting varieties. 
    - Independent study with Marston Xue (Spring 2021). Topic: Scheme theory. 
    - Senior thesis with Ying Wang (2020–2021). Topic: Grassmannian. 
    - Senior thesis with Kadin Mangalik (2019–2020). Topic: Sums of cubes over $\mathbb{F}_p$.
    - Research project with Cinea Jenkins (Summer 2019). Topic: Hodge polygons of F-crystals.
    - Independent study with Ying Wang (Spring 2019). Topic: Category theory. 
    - Senior thesis with Zhaopeng Li (2018–2019). Topic: Sums of cubes over $\mathbb{F}_p$.
* UC Berkeley 
    - Directed reading program with Frank Dai (Fall 2017). Topic: Class field theory.
    - Directed reading program with Alice Zhao (Fall 2014). Topic: Elliptic curves.
